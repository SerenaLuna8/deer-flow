...
class UnstructuredEpubExtractor(BaseExtractor):
    ...
    def extract(self, setting: ExtractSetting, context: ExtractionContext) -> list[Document]:
        ...
        prepare_local_parser("unstructured.epub")
        from unstructured.partition.epub import partition_epub

        elements = partition_epub(filename=str(setting.source_path), languages=[""])
        context.check_cancelled()
        return elements_to_documents(elements, kind="paragraph")
