...
class UnstructuredEmlExtractor(BaseExtractor):
    @override
    def extract(self, setting: ExtractSetting, context: ExtractionContext) -> list[Document]:
        context.check_cancelled()
        prepare_local_parser("unstructured.eml")
        from unstructured.partition.email import partition_email

        elements = partition_email(filename=str(setting.source_path), languages=[""], process_attachments=False)
        context.check_cancelled()
