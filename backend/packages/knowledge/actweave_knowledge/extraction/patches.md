...
| `text_extractor.py` | `builtin/text_extractor.py` | Remove host imports; bounded encoding detection and strict decoding. | P1-T3 / text and encoding gate |
| `markdown_extractor.py` | `builtin/markdown_extractor.py` | Preserve Markdown literal angle brackets, fenced/inline code and MDX source; execute nothing; do not fetch remote images. | P1-T3 / Markdown literal preservation gate |
| `pdf_extractor.py` | `builtin/pdf_extractor.py` | Remove Dify cache/storage/database binding; P2 owns cache. Use injected image sink, retain page spans; no OCR or image-text substitution. | P1-T6 / PDF pages, images and no-indexable-text gate |
| `word_extractor.py` | `builtin/word_extractor.py` | Remove host file/storage imports; retain body order, nested-table order and significant spaces; image occurrences use the injected sink. | P1-T6 / Word mixed/nested-order and image gate |
| `excel_extractor.py` | `builtin/excel_extractor.py` | Retain empty columns and original sheet/row/column positions; deterministic per-sheet headers. XLS and XLSX have separate image capabilities. | P1-T4 / Excel field-binding, location and XLS gate |
| `csv_extractor.py` | `builtin/csv_extractor.py` | Keep strings/leading zeros, reject malformed rows, bounded encoding detection; first ten rows only for automatic header selection. | P1-T4 / CSV string, bad-row and header gate |
| `html_extractor.py` | `builtin/html_extractor.py` | Remove active HTML safely while keeping structure/text; forbid URL fetching, scripts and external images. | P1-T3 / HTML structure and no-fetch gate |
