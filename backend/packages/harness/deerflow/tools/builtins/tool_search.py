...
- build_deferred_tool_setup: assembles the catalog + tool from an Agent's
  candidate tools.
- build_mcp_routing_middleware: builds the PR2 auto-promote middleware from
  serialized routing metadata on deferred candidate tools.
...
class DeferredToolSetup:
    ...
    - **Empty** ``(None, frozenset(), None)``: deferral is disabled, or no MCP
      candidate tool exists. Nothing is deferred — bind tools as-is.
...
def build_deferred_tool_setup(candidate_tools: list[BaseTool], *, enabled: bool) -> DeferredToolSetup:
    ...
    Lead Agents pass their full configured candidate list. Active-Skill policy
    later filters model-visible schemas, execution, and ``tool_search`` results.
    Subagents may pass an already statically filtered list because their Skills
...
def assemble_deferred_tools(candidate_tools: list[BaseTool], *, enabled: bool) -> tuple[list[BaseTool], DeferredToolSetup]:
    ...
    Fail closed on deferred assembly: if tool_search is enabled and MCP
    candidates exist but no deferred set was recovered, raise rather than
    silently binding their full schemas to the model. Lead authorization is
    enforced separately by ``SkillToolPolicyMiddleware``.
...
def build_mcp_routing_middleware(
    ...
    *,
    ...
    """Build PR2 auto-promotion middleware from deferred candidate tools.
