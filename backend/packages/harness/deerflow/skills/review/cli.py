...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("target", help="Skill directory or .skill archive")
    parser.add_argument(
        "--profile",
        choices=["deerflow", "agentskills"],
        default="deerflow",
    )
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "--max-files",
        type=int,
        default=DEFAULT_PACKAGE_LIMITS.max_files,
    )
    parser.add_argument(
        "--max-file-bytes",
        type=int,
        default=DEFAULT_PACKAGE_LIMITS.max_file_bytes,
    )
    parser.add_argument(
        "--max-total-bytes",
        type=int,
        default=DEFAULT_PACKAGE_LIMITS.max_total_bytes,
    )
    args = parser.parse_args(argv)
    if min(args.max_files, args.max_file_bytes, args.max_total_bytes) < 1:
        parser.error("review limits must be positive")
...
def _exit_code(
    ...
    *,
    ...
    return int(any(SEVERITY_RANK.get(str(finding.get("severity")), 99) <= threshold for finding in facts.get("findings", [])))
