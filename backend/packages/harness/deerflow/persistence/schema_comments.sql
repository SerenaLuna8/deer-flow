...
-- Schema shape SHA-256: fc7dc5f0c92c5841d5346cdc37f2975e64dcc7b8464c301243ee90fb3d18e69b
-- Comment statements SHA-256: 83e142880bf5f9d33e4ff97d208686aa3ca4517faf080feec1a8d2108d9d8044
-- Coverage: 113 static tables and 1450 columns.
...
COMMENT ON TABLE system_model_configs IS '保存系统模型的稳定配置、能力和当前秘密 Generation 指针；逻辑删除后仍可解析已接纳运行。';
...
COMMENT ON COLUMN system_model_configs.deleted_at IS '系统模型配置：不可逆逻辑删除时间；置值后从目录和新运行中隐藏。';
...
COMMENT ON TABLE model_providers IS '保存宿主管理的模型端点、请求超时与当前加密 API Key；逻辑删除后仅保留历史引用。';
...
COMMENT ON COLUMN model_providers.deleted_at IS '模型供应商：不可逆逻辑删除时间；置值后保留历史模型关联。';
...
COMMENT ON TABLE model_provider_models IS '保存供应商下的 embedding 或 rerank 具体模型及其维度、批量与状态；逻辑删除后仅保留历史引用。';
...
COMMENT ON COLUMN model_provider_models.deleted_at IS '供应商模型：不可逆逻辑删除时间；置值后从检索模型目录和新绑定中隐藏。';
...
COMMENT ON COLUMN knowledge_bases.default_relative_cutoff IS '知识库：可选的相对分数截断：低于本库最高原生分数该比例的候选被淘汰；空表示不启用。';
COMMENT ON COLUMN knowledge_bases.chunking_mode IS '知识库：库级分段模式（general 或 parent_child）：由首个文档确定，后续上传与单文档重新解析必须一致，仅整库重新解析可切换；尚无文档时为空。';
...
COMMENT ON COLUMN knowledge_tasks.kind IS '知识后台任务：任务类型（摄取文档、重嵌入文档、生成片段摘要、重建词法索引、删除文档、清理晚到文档对象、删除提取结果或删除知识库）。';
COMMENT ON COLUMN knowledge_tasks.target_version IS '知识后台任务：ingest_document、reembed_document、summarize_document 与 relex_document 任务允许发布的文档版本号。';
