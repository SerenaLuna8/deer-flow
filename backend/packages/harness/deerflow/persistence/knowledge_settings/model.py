...
class KnowledgeSystemSettingsRow(Base):
    ...
    etl_type: Mapped[str] = mapped_column(String(32), nullable=False, default="builtin", server_default=text("'builtin'"))
    extraction_cache_enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default=text("true"))

    __table_args__ = (
        CheckConstraint("etl_type IN ('builtin', 'unstructured_local')", name="ck_knowledge_system_settings_etl_type"),
