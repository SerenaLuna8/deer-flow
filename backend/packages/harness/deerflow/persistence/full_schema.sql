...
COMMENT ON COLUMN system_model_configs.provider_id IS '系统模型配置：所属模型供应商标识；API Key 由供应商行统一配置。';
...
COMMENT ON TABLE knowledge_documents IS '保存上传文件、MinIO object key、切分参数、处理版本和当前处理状态。';
COMMENT ON COLUMN knowledge_documents.id IS '知识文档：主键标识。';
COMMENT ON COLUMN knowledge_documents.project_id IS '知识文档：所属项目标识。';
COMMENT ON COLUMN knowledge_documents.knowledge_base_id IS '知识文档：所属知识库标识。';
COMMENT ON COLUMN knowledge_documents.name IS '知识文档：用户看到的文档名称。';
COMMENT ON COLUMN knowledge_documents.original_name IS '知识文档：上传文件的原始文件名。';
COMMENT ON COLUMN knowledge_documents.storage_key IS '知识文档：由软件包生成、位于配置 MinIO bucket 内的 object key。';
COMMENT ON COLUMN knowledge_documents.media_type IS '知识文档：上传时记录的媒体类型。';
COMMENT ON COLUMN knowledge_documents.size_bytes IS '知识文档：上传文件字节数。';
...
COMMENT ON COLUMN knowledge_documents.remove_extra_spaces IS '知识文档：预处理规则，压缩连续空白与三个以上连续换行；上传时固化。';
COMMENT ON COLUMN knowledge_documents.remove_urls_emails IS '知识文档：预处理规则，移除正文中的 URL 与邮箱地址；上传时固化。';
COMMENT ON COLUMN knowledge_documents.chunking_mode IS '知识文档：分块模式（general 平铺或 parent_child 父子分块）；上传时固化。';
COMMENT ON COLUMN knowledge_documents.child_chunk_size IS '知识文档：parent_child 模式下子块的目标最大字符数；上传时固化。';
COMMENT ON COLUMN knowledge_documents.child_chunk_separator IS '知识文档：parent_child 模式下子块切分分隔符（转义形式存储）；上传时固化。';
COMMENT ON COLUMN knowledge_documents.segment_count IS '知识文档：当前已发布版本的片段数量。';
COMMENT ON COLUMN knowledge_documents.word_count IS '知识文档：当前已发布版本全部片段的字符数合计。';
COMMENT ON COLUMN knowledge_documents.hit_count IS '知识文档：历史检索命中累计（各片段命中自增之和，片段删除不回写）。';
COMMENT ON COLUMN knowledge_documents.doc_metadata IS '知识文档：按知识库元数据字段名称键存的文档元数据值 JSON 对象；字段改名或删除时同步重写。';
...
COMMENT ON COLUMN knowledge_metadata_fields.project_id IS '知识库元数据字段：所属项目标识。';
COMMENT ON COLUMN knowledge_metadata_fields.knowledge_base_id IS '知识库元数据字段：所属知识库标识。';
COMMENT ON COLUMN knowledge_metadata_fields.name IS '知识库元数据字段：知识库内唯一（忽略大小写）的元数据字段名称。';
COMMENT ON COLUMN knowledge_metadata_fields.field_type IS '知识库元数据字段：字段类型（string、number 或 time；time 值按 epoch 秒存储）。';
COMMENT ON COLUMN knowledge_metadata_fields.created_at IS '知识库元数据字段：记录创建时间。';
COMMENT ON COLUMN knowledge_metadata_fields.updated_at IS '知识库元数据字段：记录最近更新时间。';

COMMENT ON TABLE knowledge_segments IS '保存知识文档当前已发布版本的有序文本块及其向量。';
COMMENT ON COLUMN knowledge_segments.id IS '知识片段：主键标识。';
COMMENT ON COLUMN knowledge_segments.project_id IS '知识片段：所属项目标识。';
COMMENT ON COLUMN knowledge_segments.knowledge_base_id IS '知识片段：所属知识库标识。';
COMMENT ON COLUMN knowledge_segments.knowledge_document_id IS '知识片段：所属知识文档标识。';
COMMENT ON COLUMN knowledge_segments.document_version IS '知识片段：生成本片段时的文档处理版本号。';
COMMENT ON COLUMN knowledge_segments.position IS '知识片段：片段在文档版本内从 1 开始的位置。';
COMMENT ON COLUMN knowledge_segments.content IS '知识片段：用于检索和引用展示的文本内容（属于私有内容）。';
COMMENT ON COLUMN knowledge_segments.word_count IS '知识片段：内容字符数。';
...
COMMENT ON COLUMN knowledge_segment_children.project_id IS '知识子块：所属项目标识。';
COMMENT ON COLUMN knowledge_segment_children.knowledge_base_id IS '知识子块：所属知识库标识。';
COMMENT ON COLUMN knowledge_segment_children.knowledge_document_id IS '知识子块：所属知识文档标识。';
COMMENT ON COLUMN knowledge_segment_children.knowledge_segment_id IS '知识子块：所属父级知识片段标识。';
COMMENT ON COLUMN knowledge_segment_children.document_version IS '知识子块：生成本子块时的文档处理版本号。';
...
COMMENT ON COLUMN knowledge_tasks.claim_token IS '知识后台任务：当前 Worker 领取的随机 token；非 running 时为空。';
COMMENT ON COLUMN knowledge_tasks.lease_until IS '知识后台任务：当前领取租约到期时间；过期后任务可重新领取。';
COMMENT ON COLUMN knowledge_tasks.error_message IS '知识后台任务：最近一次执行失败的可展示错误。';
