...
compare candidates with the same ActWeave workload.
...
same ActWeave workload has been measured on both the current AIO sandbox and the
candidate backend.

For an experimental provider PR, prefer `Related to #3213` unless the PR also
includes reproducible ActWeave workload data that demonstrates the target memory
