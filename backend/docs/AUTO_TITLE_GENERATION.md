...
4. `title.model_name` 为空时，直接从首条用户消息生成本地 fallback，不增加一次 LLM 调用。
5. 配置 `title.model_name` 后，middleware 异步调用指定模型；模型失败或返回空标题时回退到本地标题。
...
- `max_chars`：最终标题长度上限。
- `model_name`：可选模型名；`null` 使用本地 fallback。
- `prompt_template`：可选自定义 prompt，必须兼容当前配置模型要求。
