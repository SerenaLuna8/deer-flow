...
- M5 Automation completion and restart recovery are owned by `AutomationReconciler`, not callback metadata or the legacy scheduler repositories. Completion treats the persisted M4 Run as authority, re-derives project+owner scope, locks project/membership -> definition -> occurrence -> Run, and increments the parent count only after the first terminal occurrence CAS. In supported single-Gateway startup, automation restart reconciliation runs before generic M4 orphan recovery: an expired `launching` occurrence is requeued only when no deterministic private Run exists, while an admitted active Run is interrupted and never replayed. The occurrence-driven `ScheduledTaskService` then reserves, claims, and dispatches through the M4 private runtime; disabled, multi-worker, incomplete-cutover, and unavailable startup paths remain initialized but do not poll. Shutdown cancels the scheduler before channels and runtime dependencies close.
