...
make backup-db ARGS="--output /secure/backups --source-installation-id <id>"  # encrypted PostgreSQL archive; requires DATABASE_URL and DEER_FLOW_BACKUP_KEY
...
- M6 Task 16's `app.recovery.archive` is an operator-only archive primitive, not a restore service. `make backup-db ARGS="--output /secure/backups --source-installation-id <id>"` reads `DATABASE_URL` and the independent base64 32-byte `DEER_FLOW_BACKUP_KEY` from the environment, invokes only fixed `pg_dump --format=custom --no-owner --no-acl` argv, and streams its custom-format bytes into unique-nonce AES-GCM chunks. AAD binds archive ID, schema revision, source installation ID, and chunk index; a separately derived manifest HMAC covers all metadata and ciphertext hashes. Reader authentication is all-or-nothing: no plaintext is yielded until every chunk is verified. The archive root/directories are 0700, files 0600, and a staging directory is atomically renamed only after manifest finalization. Command output is restricted to public archive ID/revision/count/checksum; it excludes URLs, locations, keys, plaintext, and subprocess output. Restore, recovery journal, retention purge, and drills remain Task 17 work.
