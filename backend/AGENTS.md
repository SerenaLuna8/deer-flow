...
**Architecture**:
- **Gateway API** (port 8001): project/account/admin REST API and private Run admission
- **Frontend** (port 3000): Next.js web interface
- **Nginx** (port 2026): Unified reverse proxy entry point
- **Provisioner** (port 8002, optional in Docker dev): Started only when sandbox is configured for provisioner/Kubernetes mode

**Runtime**:
- M7 removes the Gateway global agent runtime and `/api/langgraph/*` rewrite. Nginx sends `/api/*` directly to Gateway; project-private Runs are admitted durably by Gateway and executed only by the independent Worker.
- Under final M6 cutover, private and Automation runs execute only in the independent Worker through the same `RunAgentPrivateExecutor`; the independent Scheduler only admits durable jobs and never imports the Worker executor or Gateway runtime.
...
make worker             # Run the independent M6 Worker role
make scheduler          # Run the independent M6 Scheduler role
...
| **Project private work** (`/api/projects/{id}/private-work`) | Project-scoped Thread, Run admission/reconnect/wait, feed, feedback, upload/file, artifact, Memory, token-usage, and input-polish operations. Input polish requires `private_work.create` plus `shared_assets.execute`, locks the scoped Thread, and validates its server-resolved Agent snapshot and Credential-grant closure before the one-shot auxiliary LLM call. No global Thread/Run/Memory/input-polish compatibility surface remains. |
| **Project connections** (`/api/projects/{id}/connections`) | Strict project-only provider discovery, list/connect/disconnect APIs. Provider discovery exposes availability metadata only; no credential fields or values. The global channel/channel-connections and console APIs are removed. |
| **System operations** (`/api/admin/operations`) | System-admin aggregate readiness, counts, quota usage, and bounded channel-provider health. Provider health is reduced to provider/status/checked-at/safe-code and never includes tokens, webhook secrets, external account identifiers, or connection payloads. |
...
- M6 changes `start_private_run()` into an admission-only Gateway boundary: it sanitizes client fields, atomically persists the pending Run, exact asset snapshot, durable `private_run` job, and transaction-bound quota/audit hooks, then returns a store-only `RunRecord` without touching the checkpointer, `RunManager`, asset materializer, or `asyncio.create_task`. A deterministic retry may adopt only the same scoped Run/job pair with identical sanitized request data; hooks are not charged twice. Worker execution/cancel/checkpoint takeover and Gateway reconnect/wait consumption are implemented. Project SSE accepts only canonical non-negative ASCII `Last-Event-ID` values (empty means zero), replays PostgreSQL pages before polling, emits every durable frame ID including terminal, and returns an SDK-relative `Location` for reconnect; malformed/ahead cursors fail with `INVALID_STREAM_CURSOR`, while 429/503 responses carry `Retry-After`. `app/gateway/routers/private_work.py` is mounted at `/api/projects/{project_id}/private-work` for readiness, Thread, run/stream/wait, feed, file, and artifact operations. Gateway lifespan initializes the project-scoped checkpointer plus private Thread, Run, file, and durable PostgreSQL stream services on `app.state`. It also installs a dedicated PostgreSQL `private_run_event_store`; project workers and feed endpoints use it regardless of the legacy `run_events.backend`, so the default in-memory legacy store cannot erase project chat history on restart. M7 Task 3 removes the global Thread/run/Memory/upload/artifact routers and shared Gateway execution runtime; project readiness is governed by the final-schema probe. The historical migration scripts remain operator-only staging tools and are not runtime API authority.
...
- M5 Automation dispatch keeps Task 5 claim rows free of speculative runtime foreign keys. Task 6 derives deterministic Thread/Run IDs, creates or strictly adopts a real scoped M4 Thread, starts the Run only through `start_private_run()`, and atomically backfills both real foreign keys while moving `launching` to `running`. Fresh and reuse modes both fail closed on scope, Agent, version, metadata, or current-authority drift. Once a matching M4 Run exists, the occurrence is never requeued with null runtime references, and dispatch never falls back to shared `start_run()`. A post-admission launcher error re-reads that exact Run under the normal lock order: terminal status uses the same public-safe outcome mapper as reconciliation and first-CAS parent settlement, while `pending`/`running` backfills the occurrence to `running` with the durable Run timestamps and keeps global capacity occupied. Only `AutomationUnavailable` with no matching Run may requeue; mismatched Run coordinates remain fail-closed.
- M6 Automation admission is owned by `AutomationDispatcher.admit_occurrence()`: one transaction locks project/membership -> definition -> occurrence -> Thread -> Run/snapshot -> job, creates the deterministic occurrence/Run plus `automation_run` job, attaches the same job ID to both parents, and only then exposes `running`. Scheduled and manual idempotency retries adopt that exact triple even after the definition is paused or terminal; partial Thread, snapshot, Run, or job state rolls back together. Gateway owns only manual admission and never constructs a scheduler poll task. `app.scheduler` is a separate process started with `make scheduler`; it holds fixed PostgreSQL session advisory lock `0x0DEE_12F1_0A55_0007` on one dedicated connection, verifies the original backend PID and existing lock before each poll without reacquiring, scans due definitions, and only enqueues. Ownership loss permanently fail-stops the poller and exits the process lifetime so another Scheduler can take over after PostgreSQL releases the session. Worker startup always runs idempotent terminal-only Automation reconciliation before claiming jobs; enabled Scheduler startup does the same before polling. Neither path requeues a legacy `launching` row, interrupts an active Worker Run, or replays admitted work. Disabled Scheduler mode takes no engine lock or poll task while Gateway manual APIs and Worker restart reconciliation remain available.
- M5 Automation authorization retention extends the existing project/member governance transaction instead of adding a parallel lifecycle. Membership removal/leave, non-Viewer→Viewer downgrade, project suspension, and pending deletion preserve project -> membership -> definition -> occurrence -> Run locking by freezing the exact project+owner Automation scope before writing the M4 Run cancellation marker and committing: non-deleted definitions become `paused`, clear `next_run_at`, set `frozen_at`, and advance their version; queued occurrences become terminal `cancelled`, while `launching`/`running` occurrences settle only through the authoritative M4 Run marker plus `AutomationReconciler`. Every dispatcher/restart requeue locks and revalidates that same project, membership, definition, occurrence, and optional Run sequence in one transaction; inactive/suspended/revoked/frozen/deleted or version-drifted state terminalizes the launch instead of returning it to `queued`, so it cannot retain global active capacity. Restore, resume, and invitation rejoin clear `frozen_at` only for active owners' non-deleted definitions and deliberately leave them `paused` with no next tick. A later explicit owner `resume()` must revalidate current capability/assets/thread/once constraints and compute a new future tick. The existing post-commit local cancellation notification remains acceleration only, never transaction authority.
- `app.automations` owns the public-safe Automation error mapping, frozen application DTOs, and read-only final-schema readiness result. Marker-derived Automation cutover errors and branches are gone; schema/repository failures fail closed with stable public codes. `scheduler.enabled=false` reports scheduler availability separately and never closes an otherwise-ready project Automation API.
...
- `app.recovery.archive` is the operator-only M7 archive primitive, not a compatibility restore service. `make backup-db ARGS="--output /secure/backups"` reads `DATABASE_URL`, the independent base64 32-byte `DEER_FLOW_BACKUP_KEY`, and the existing audit HMAC environment. Before `pg_dump` it requires the exact M7 root-object inventory, revision `0001_project_saas_baseline`, and canonical catalog digest; the cancellation-safe read-only repeatable-read exported snapshot verifies that exact root inventory, revision, and catalog again in the same transaction and supplies source identity, event high-watermark, contiguous tombstone prefix, and table count. Fixed password-free `pg_dump --format=custom --no-owner --no-acl --snapshot=...` receives only an inherited `/dev/fd/N` passfile descriptor plus `pass_fds`. Archive schema version is fixed at 7; one strict frozen manifest model rejects coercion and unknown fields for both writer and reader, and per-chunk AAD binds archive ID, schema version, revision, canonical digest, source identity, and chunk index. Authentication precedes old-shape classification: a re-signed manifest with changed schema fields fails chunk authentication, while an exact fully authenticated pre-M7 shape fails closed as `UNSUPPORTED_ARCHIVE_SCHEMA`. The writer and reader retain the 16 MiB manifest and 65,536 one-MiB chunk limits, two ciphertext authentication passes, bounded `0600` plaintext spool, fd-relative no-follow publication, cancellation-settled identity-safe cleanup, key separation, and mandatory trusted `backup.created` audit commit. The former pre-M6 cutover producer/receipt mode and its CLI flag no longer exist. Public output is limited to archive ID, schema version/revision/digest, counts, and truncated checksum; URLs, keys, plaintext, and subprocess output remain forbidden.
...
- `make restore-db ARGS="--archive ... --target-url ... --journal ... --execute"` authenticates the complete archive and enforces M7 version/revision/digest before target parsing, existence checks, creation, or DDL. It then requires the source to remain exact M7 and holds the same source PostgreSQL session advisory authority through source/archive/journal identity and anchor verification, target creation, fixed `pg_restore --exit-on-error --no-owner --no-acl`, exact frozen-suffix replay, the same exact M7 schema verifier, sensitive cleanup, proof, and explicit unlock. A concurrent purge therefore waits and cannot be omitted from a claimed proof. Anchor failure/cancellation still performs a cancellation-settled explicit unlock; cancellation received while unlocking is rethrown only after reliable release, and a proof already committed by that invocation does not permit a verified return—the invocation-owned target is removed. `restore_proofs` binds the archive schema version, canonical schema digest, frozen source identity, journal UUID, final sequence, and final head digest; database constraints require the exact M7 version, revision, and canonical digest. To avoid digest self-reference, catalog hashing replaces that literal with a fixed placeholder only when the complete constraint text contains the current canonical digest; another valid lower-hex literal is never normalized and fails exact-schema verification. The authenticated dump, libpq passfile, and invocation-owned workspace use separately captured dev/inode identities, bounded cancellation-safe deletion, absence checks, and directory fsync before proof. Body failure cleans target and workspace before leaving source authority. Cleanup accepts only the exact captured name→identity set; an unknown file is neither adopted nor deleted, blocks proof, and makes the operation fail closed. Restore accepts only a nonexistent distinct `deerflow_restore_<pid>_<32hex>` database and never changes `DATABASE_URL`, starts services, or cuts traffic. `make drill-restore ARGS="--archive ... --journal ..."` uses the same workflow for one random database and drops it only after the same Restorer instance hands off an unforgeable verified ownership token; failure before create, an existing/racing target, or a forged result never authorizes DROP. CLI output contains only public proof metadata.
...
- `GET /api/projects/{project_id}/usage`, `PATCH /api/projects/{project_id}/usage/limits`, and `GET /api/projects/{project_id}/audit` are the Task 14 project-governance surface. Every request resolves a fresh immutable `ProjectContext`, requires the exact `PROJECT_USAGE_READ` or `PROJECT_AUDIT_READ` capability, and rechecks `FinalSchemaProbe.require_ready()` before data access. Usage returns configured/effective limits plus zero-filled current used/reserved counters and 80% warning state; quota updates use an issued `PrivateWorkContext`, optimistic versioning, and the caller-owned transaction for both `QuotaService.set_limits()` and `OperationalAuditSink.quota_policy_updated()`. Every session-bound `AuditService.list_project()` call independently locks and revalidates exact current Project→Membership authority—including user/project/membership IDs, membership version, active state, role, and capabilities—before the descending audit query, so callers cannot reuse a context retained before suspension, removal, or downgrade. The HTTP route may repeat that validation inside its caller transaction; `list_project_new_session()` remains the compatibility entry, opens its own transaction, and delegates while those locks survive listing. Audit responses expose only allowlisted actor/action/target/outcome/public metadata fields; target HMACs, raw private identifiers, and secret material remain internal. Missing or stale authority, outsider access, and cross-project access all collapse to public 404.
- `ProjectQuotaEnforcer` is installed in Gateway, Worker, and Scheduler with one deployment-stable, domain-separated HMAC keyring. Member create/redeem/end (including first-admin default-project bootstrap), file finalize/delete/branch copy/Run finalization/Thread delete, private and Automation Run admission, queued cancel, Worker settlement, unowned job terminalization, and each actual MCP network dispatch mutate quota in the authoritative transaction. Retry-wait retains its Run reservation; every terminal path performs the same exact idempotent release. Hard-limit HTTP responses are stable 429 contracts with `Retry-After: 1`; lowering a policy blocks only new reservations and never interrupts admitted work. All three roles require `DEER_FLOW_AUDIT_ACTIVE_KEY_ID` plus canonical 32-byte `DEER_FLOW_AUDIT_KEYRING_JSON`; the same secret material is domain-separated for job-owner and quota-source references and never persisted. Rotation may change the active ID only after every role has the same retained old+new key set; exact reserve/release and replays search all retained refs while new ledger rows use the active key. Keep old material until no retained ledger/audit row references its ID.
- M6 audit persistence is allowlist-first and caller-transaction-bound. `AuditService.append()` accepts only exact `AuditAction`, `AuditActor`, `AuditTarget`, and `AuditOutcome` contracts, binds every action to immutable joint variants of target kind + project/platform scope + actor kind + exact process + metadata discriminator, then validates metadata against the action-specific strict Pydantic schema with unknown fields forbidden. The joint variant prevents a project user from writing a platform asset event, binds manual/scheduled Automation and private/Automation Run admission to the matching actor/job metadata, requires project targets to hash the same UUID as `project_id`, and explicitly distinguishes project/file purge from platform account purge. `OperationalAuditSink` and `TrustedOperationAuditSink` require an opaque process context issued once by the same `AuditService` through a fixed-role Gateway/Worker/Scheduler/Operator/Recovery composition binder; process actor registry entries carry that service issuer and `append()` rejects actors minted by any temporary or different service. Contexts cannot be constructed/copy-forged/mutated/cross-service reused; generic process binders, arbitrary-role test binders, and public mutable registries are forbidden. Queued Gateway cancellation is the only Gateway-authored `run.terminal` variant. Gateway project/Automation/shared-asset mutation dependencies return 503 when their durable audit sink is absent; Automation definition-service and manual-trigger dispatcher dependencies share one private mapper for the stable `AUTOMATION_UNAVAILABLE` code/message/request-id envelope, while project and shared-assets retain their generic 503 contract. Compatibility apps must explicitly inject a test sink. The M3-compatible shared-asset adapter rechecks an override actor's current `system_admin` database role or a project actor's active membership before minting the formal actor. Project create/update, lifecycle delete/recover/suspend/resume, invitation create/revoke/redeem plus member join, membership changes, and Automation create/update/pause/resume/delete invoke typed sink methods before the caller transaction commits; failed mutations roll back their audit and same-role updates write neither a version nor an audit row. Safe system requeue accepts only project+dead coordinates, derives owner/run scope inside the harness repository, returns a repository-issued public-safe event, and lets the sink atomically consume that event once without loading a naked Job; repository cleanup consumes any capability an arbitrary callback retained before returning or raising. `system_admin` writers must derive their actor from a registry-verified issued system audit context; direct elevated actor construction is rejected. Private target UUIDs are persisted only as a separately domain-separated HMAC plus key ID; readers search the active and every retained rotation ref, never expose the digest, and scope project reads by `project_id` plus the target digest when present. Request correlation accepts the existing normalized 512-character trace contract but persists only a separate-domain HMAC, never the client-controlled `X-Trace-Id`. Project reads require `PROJECT_AUDIT_READ`; platform reads require a registry-verified system-admin audit context that cannot be constructed or forged directly. The repository never commits, project/platform pagination uses descending `(occurred_at,id)` cursors, and final-schema triggers reject update/delete of committed audit rows. Do not add independent actor/scope allowlists, arbitrary metadata dictionaries, raw target IDs/names/paths/content, raw trace IDs, exception text, free-text search, repository-owned commits, public process binders, unscoped platform contexts, or mutation-based corrections; corrections append `audit.corrected`.
- M6 durable jobs use the session-bound `JobRepository`; callers own every transaction. Enqueue uses `(job_type,idempotency_key)` conflict handling but revalidates project/owner/run/occurrence authority before adopting an existing row. Claims order eligible jobs deterministically, preselect coordinates, and lock Project→Membership before the exact `FOR UPDATE SKIP LOCKED` Job; a savepoint releases authority locks for a skipped candidate. Owned/unowned terminal transitions and private Run settlement preserve Project→Membership→Job→Run→File→quota counter, including revoked-member compensation and branch rollback. Each claim creates a monotonic attempt plus a 32-byte random lease token whose SHA-256 hash is the only persisted form. Running, heartbeat, success, cancellation, retry, and dead transitions all compare that hash and reject expired/stale owners without mutation. A successful heartbeat also reports a cancellation requested after claim. Expired safe leases may be reclaimed within the attempt bound; unknown/unsafe ambiguity or exhaustion writes one append-only `dead_jobs` projection. Private dead projections require an injected owner HMAC before any transition. Owner-scoped safe requeue remains compatible, while `requeue_safe_system()` accepts only an exact parentless `retention_purge` predecessor: it never rebinds a private Run or Automation occurrence. The final database schema enforces one successor per `predecessor_dead_job_id`; repository retries adopt that exact valid successor even with a different idempotency key and emit no second audit event. There is no independently callable requeue signer: the repository locks and revalidates both safe dead predecessor and newly created queued safe successor before lexically issuing the opaque event and invoking audit in the same transaction. The event registry is an atomic one-shot capability: the first sink validation pops its weakref+snapshot entry, repository `finally` cleanup revokes it even if a retaining callback raises and the successor transaction rolls back, and a later business retry creates and issues a new event rather than restoring the old authority. Worker dead settlement carries an explicit `run_terminal_published` result from the job terminal port through private Run settlement, so the terminal port or handler owns `run.terminal`, never both. Never expose owner/run through the requeue event, load a naked Job in its sink, add a module signer, add repository-owned commits, persist raw lease tokens, reverse lock acquisition, perform unscoped cancellation/dead lookup, update dead rows, or automatically requeue unknown/unsafe jobs.
- `PrivateRunAdmissionService` composes `PrivateRunRepository`, `RunSnapshotRepository`, and `PrivateRunJobRepository` inside one caller-owned transaction. New Runs are inserted before their job to satisfy the exact-scope job foreign key, then `runs.job_id` is attached before quota reservation and the audit hook; any hook or database failure rolls back Run, snapshots, job, attachment, quota ledger, and audit row together. Gateway installs both persistent quota and audit ports; their default no-ops remain only for isolated compatibility tests. Job IDs remain server authority and are not added to the public Run response.
- The independent M6 Worker lives under `app.worker` and is started with `make worker`; it must not import Gateway routers or launch inside Gateway lifespan. `WorkerRegistry` stores only a random Worker UUID, bounded version/capabilities/capacity, timestamps, and draining state—never hostname, environment, URL, PID, or credentials. `WorkerService` claims only registered handler capabilities, bounds in-flight tasks by `worker.max_concurrent_jobs`, extends each lease through its token-authorized heartbeat, accepts only uppercase stable public error codes, and uses truncated SHA-256 task labels instead of raw Job IDs. Shutdown first stops claims and marks the node draining, then allows the bounded grace plus bounded cancellation window; unfinished local tasks have their authority invalidated before detach and cannot heartbeat or settle, so PostgreSQL lease expiry remains the only takeover authority. An uncooperative handler after runtime lease loss fail-stops new claims instead of freeing capacity. `remove` is a logical fleet removal that retains the draining Worker row because immutable `job_attempts` history has an `ON DELETE RESTRICT` reference. Registry loss, stale tokens, or heartbeat database failure—including a handler's explicit heartbeat—cancel local execution and never forge a terminal state.
- M6 private Run execution is Worker-only. `PrivateRunJobHandler` locks the exact project+owner+Run+job pair, mirrors the current durable job lease into the Run, reloads the persisted asset/credential snapshot, and returns a deferred `JobSettlement`; `WorkerService` stops the heartbeat before that callback atomically settles both Run and job. `RunAgentPrivateExecutor` is the sole M6 adapter that invokes `run_agent()`. It uses a process-local `RunManager` with no Run store so harness status updates cannot bypass durable lease authority, while `PrivateRunExecutionBoundary` revalidates both membership/capabilities and the current job/Run token before model, tool, MCP, sandbox, checkpoint, stream publication, and finalization operations. MCP discovery/materialization remains retry-safe; the exact network dispatch first consumes quota, then durably marks `retry_safety=unknown`, so a quota rejection retains `PROJECT_MCP_QUOTA_EXCEEDED` and never masquerades as an ambiguous side effect. The raw shared stream bridge is never passed to `run_agent`; stale attempts cannot publish data/END or delete a successor's stream. The Worker always constructs `PostgresStreamBridge`: each frame commits to scoped `run_events(category='stream')` before optional best-effort notification, IDs are thread-monotonic decimal `seq`, readers carry project+owner+thread predicates, and one partial unique index plus the per-thread advisory writer lock makes `stream.end` idempotent under concurrent attempts. Job-owned appends accept only the ephemeral raw-token `StreamLeaseProof`; after waiting for the thread lock, the event store locks and validates current Project, Membership, Job, Run, token hash, expiry, status, and cancellation in the same append transaction. Arbitrary callbacks, naked `run_id` publish/subscribe, and generic `RunEventStore` stream writes all fail closed. Durable cleanup is retention-owned and never deletes frames on the Worker hot path. Sandbox acquisition is authorized before provider allocation, while deterministic snapshot restore remains retry-safe and does not mark the job ambiguous. A reclaimed safe job reuses the same Run, checkpoint namespace, and exact snapshot; each attempt records its starting checkpoint cursor and only an advanced durable cursor suppresses replay of the original input/Command. A persisted durable terminal is the final stream outcome and wins over a cancellation arriving afterward, keeping SSE replay and Run/job state identical. A stale token cannot heartbeat or publish terminal state. Transient failures return the same Run to `retry_wait`; an external tool/MCP/agent sandbox mutation/file boundary first persists `retry_safety=unknown`, so a crash or later failure becomes `SIDE_EFFECT_STATE_UNKNOWN` and is never replayed. Production Worker startup requires the separate `DEER_FLOW_AUDIT_ACTIVE_KEY_ID` + canonical 32-byte `DEER_FLOW_AUDIT_KEYRING_JSON`; private dead projections store only that key ID and a domain-separated owner HMAC. The job terminal port settles unowned dead/cancelled Runs and removes their exact staging files in the same transaction. For Automation parents it uses non-blocking Task -> occurrence try-locks after Run settlement; contention never reverses the reconciliation lock order, and Worker/Scheduler terminal-only reconciliation completes any deliberately deferred parent outcome. Project cancel requests persist on both Run and job; queued work settles immediately, late leased cancellation is observed by heartbeat and sets the local abort event, while non-cancel terminal Runs return conflict. The private LangGraph-compatible cancel route supports durable interrupt with bounded disconnect-aware waiting and explicitly rejects rollback until a checkpoint-restore job exists. Graph `input` and `command` payloads are preserved byte-for-structure during admission because keys such as message `role` are state data, never authority; metadata and runtime config/context remain recursively sanitized.

Proxied through nginx: `/api/*` → Gateway REST APIs. Agent execution is Worker-only.
...
- Scheduled background runs set `context.non_interactive=true` and therefore exclude `ask_clarification` from the lead-agent tool list. This keeps scheduler-triggered runs from stalling on human confirmation mid-execution. `non_interactive` is internal-only authority: ordinary HTTP/IM payloads cannot set it, and project Automation persists it only from the server-owned admission path.
- Under M6, `AutomationDispatcher.admit_occurrence()` writes the deterministic Run ID and `context.non_interactive=true` as server-owned data in the same occurrence/Run/snapshot/job transaction. Gateway and Scheduler expose neither `run_id` nor a trusted runtime context to clients; the independent Worker consumes only that persisted admission snapshot, while client-provided `non_interactive` remains discarded.
...
Bridges external messaging platforms (Feishu, Slack, Telegram, Discord, DingTalk, GitHub) to project-private durable Runs.

**Architecture**: A project-bound message is resolved through its persisted SQL connection into an immutable `PrivateWorkContext`. `ProjectInboundDispatcher` calls `start_private_run()` directly to atomically admit the Run/job, then observes durable Run state and the project-scoped checkpointer. Agent execution occurs only in the independent Worker. The removed global LangGraph-compatible Gateway API is not a channel execution path.

**Components**:
- `message_bus.py` - Async pub/sub hub (`InboundMessage` → queue → dispatcher; `OutboundMessage` → callbacks → channels)
- `store.py` - JSON-file persistence mapping `channel_name:chat_id[:topic_id]` → `thread_id` (keys are `channel:chat` for root conversations and `channel:chat:topic` for threaded conversations)
- `manager.py` - Core dispatcher. Project-bound ordinary text resolves the persisted connection through `ProjectInboundDispatcher`, admits the resolved private Thread through `start_private_run`, and returns the durable final text.
...
- `app/gateway/routers/project_connections.py` - Project-only provider discovery and connection APIs
...
3. For project-bound channel connections, the exact provider account/workspace identity resolves one connected SQL row; its `(account_id, project_id, owner_user_id, connection_id)` tuple is authoritative, active membership/project state is re-resolved, and the conversation maps to a project-private Thread. Message `owner_user_id`, `project_id`, headers, auth-disabled users, default/recent projects, and unique membership are never authority or fallback.
4. `ProjectInboundDispatcher` admits a project-private durable Run through `app.private_work.http_runtime.start_private_run` and waits on `PrivateRunService`; it never invokes an in-Gateway agent or a global Thread/Run client.
5. The independent Worker executes the admitted job under the persisted project/owner scope. The dispatcher reads the final project-scoped checkpoint and publishes the outbound reply.
...
**Owner-scoped file storage**: inbound files, uploads, and output artifacts are staged under the DeerFlow owner's bucket so they land where the agent run reads/writes (`users/{user_id}/threads/{thread_id}/user-data/{uploads,outputs}`). Project-bound processing obtains the owner only from the resolved PostgreSQL connection context; it never falls back to the external sender ID. The resolved owner is threaded as the `user_id=` kwarg through the file pipeline:
...
Project-bound execution never derives authority from the cached filesystem bucket or platform message fields. Gateway admission and Worker execution use only the resolved PostgreSQL project/owner scope.

**Configuration** (`config.yaml` -> `channels`):
- `gateway_url` remains available for auxiliary non-Run commands. Project-bound Run admission is process-local and does not use a LangGraph-compatible URL.
...
- Product methods are keyed by exact `(project_id, owner_user_id)` scope. Connect callbacks consume the stored project OAuth/connect state and re-resolve membership before writing the connection. Ordinary bound text uses durable project-private admission via `start_private_run`.
...
- Frontend APIs: `GET /api/projects/{project_id}/connections/providers`, `GET /api/projects/{project_id}/connections`, `POST /api/projects/{project_id}/connections/{provider}/connect`, and `DELETE /api/projects/{project_id}/connections/{connection_id}`.
...
- Provider-level `connection_status` reflects only the current authenticated project/owner scope. Auth-disabled mode never fabricates a connection or project binding.
...
- The global Memory HTTP router has been removed. The runnable-first private-work migrator rejects non-empty legacy Memory instead of dropping or guessing its scope; later cleanup owns any remaining file-storage migration tooling.
...
- Project Memory HTTP operations are available only under `/api/projects/{project_id}/memory` and resolve authority from the authenticated project context.
...
CI 入口 `.github/workflows/project-foundation-postgres-tests.yml` 也调用同一 Make target。变量缺失时
runner 必须在 pytest 前硬失败；本地 exact gate：
...
`POSTGRES_TEST_URL` 必须是仅供测试的 maintenance/admin URL，具备 `CREATEDB`、
...
restore/finalizer。M7 Task 3 已删除 global upload/artifact HTTP routers；所有 HTTP file/artifact
authority 都来自 project-private PostgreSQL scope。安全 header 与 limit helper 可以继续共享，但不得
重新引入 owner-scoped host-directory Gateway 路由。Thread delete/freeze 对 file/artifact 的跨资源
标记由 `thread_service` 集成；inactive Thread 下任何 file/artifact bytes 都不可读。
...
- Project private-work clients derive URLs from the current project scope and use `/api/projects/{id}/private-work/*`.
...
- Endpoint: `POST /api/projects/{project_id}/private-work/threads/{thread_id}/uploads`
...
- Project-private HTTP uploads persist scoped PostgreSQL file authority and stage bounded bytes for independent Worker restoration/finalization. Staging files are hidden and swept on Gateway startup if a hard crash leaves one behind.
...
- Project-private compaction uses the scoped Thread/checkpointer service; no
  global `/api/threads` compaction surface remains.
