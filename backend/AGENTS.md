...
- `GET /api/projects/{project_id}/usage`, `PATCH /api/projects/{project_id}/usage/limits`, and `GET /api/projects/{project_id}/audit` are the Task 14 project-governance surface. Every request resolves a fresh immutable `ProjectContext`, requires the exact `PROJECT_USAGE_READ` or `PROJECT_AUDIT_READ` capability, and rechecks `ReliabilityCutoverGuard.require_gateway_open()` before data access. Usage returns configured/effective limits plus zero-filled current used/reserved counters and 80% warning state; quota updates use an issued `PrivateWorkContext`, optimistic versioning, and the caller-owned transaction for both `QuotaService.set_limits()` and `OperationalAuditSink.quota_policy_updated()`. Audit reads lock current Project→Membership authority and call session-bound `AuditService.list_project()` before the same request transaction exits, so suspension, removal, or downgrade cannot interleave between authorization and the descending audit query; `list_project_new_session()` remains the compatibility entry and delegates to the same session-bound implementation. Audit responses expose only allowlisted actor/action/target/outcome/public metadata fields; target HMACs, raw private identifiers, and secret material remain internal. Missing authority, outsider access, and cross-project access all collapse to public 404.
