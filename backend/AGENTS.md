...
- Redis `StreamBridge` keys use a rolling retained-buffer TTL (`stream_bridge.stream_ttl_seconds`, refreshed on `publish()` / `publish_end()`) as a leak safety net, not as a run timeout. Startup orphan recovery runs only when the effective worker count (`GATEWAY_WORKERS`, plus uvicorn's `WEB_CONCURRENCY` when set) resolves to exactly one, then publishes `END_SENTINEL` and schedules stream cleanup for recovered runs. Multi-worker startup skips recovery because runs have no worker ownership/liveness lease; malformed `Last-Event-ID` reconnect values live-tail new Redis events rather than replaying the retained buffer.
