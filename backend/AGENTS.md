...
- M5 completed its Task 18 full-stack gates and independent closure review on 2026-07-16. M6 Tasks 4–13 now provide durable generic jobs, an independent Worker, Worker-only private execution, atomic Automation occurrence/Run/job admission, an independent Scheduler, PostgreSQL durable stream writer/reader/terminal invariants, Gateway SSE reconnect, frontend cursor/dedupe, enforced project quotas, and privacy-safe append-only audit sinks for every currently existing governance/run/job caller. Task 13 closed and passed two repair reviews on 2026-07-17: process actors are bound to the issuing `AuditService`, Gateway governance mutation dependencies fail closed, Automation pause/resume audit in the definition transaction, safe requeue signing is inseparable from the exact locked predecessor/new successor transaction, and production Worker dead settlement emits exactly one `job.dead` plus one `run.terminal`. Same-role membership updates are true no-ops for both version and audit. Task 14 still owns quota-policy HTTP mutation and Tasks 16–17 still own backup/restore/purge modules plus their caller wiring; no recovery placeholder module was created early. Backup/restore, remaining orchestration, and final M6 closure are still open. Follow `docs/operations/m5-automation-migration.md` for the maintenance-window cutover; never log private titles/prompts, owner maps, or full resource identifiers.
