...
- M5 completed its Task 18 full-stack gates and independent closure review on 2026-07-16. M6 Tasks 4–12 now provide durable generic jobs, an independent Worker, Worker-only private execution, atomic Automation occurrence/Run/job admission, an independent Scheduler, PostgreSQL durable stream writer/reader/terminal invariants, Gateway SSE reconnect, frontend cursor/dedupe, enforced project quotas, and the privacy-safe append-only audit foundation. Task 13 is checkpointed but not closed: transactional sinks currently cover project/system shared-asset mutations, membership changes, Run admission/cancel/terminal, Automation trigger, and job dead/requeue wiring across Gateway/Worker/Scheduler; recovery and quota-policy callers plus final independent review remain open. Backup/restore, remaining orchestration, and final M6 closure are still open. Follow `docs/operations/m5-automation-migration.md` for the maintenance-window cutover; never log private titles/prompts, owner maps, or full resource identifiers.
- M6 reliability contracts are in progress. `config.yaml` version 23 adds restart-required `worker`, `quotas`, and non-secret `recovery` policy. `app.reliability` owns the public-safe error mapping, immutable readiness result, and `ReliabilityCutoverGuard`: queue/Gateway/Worker paths re-read complete M4, M5, and M6 markers plus the M6 final revision on every boundary, while legacy in-Gateway execution closes once the M6 marker completes. Fresh empty databases write a truthful empty-domain M6 marker only after final relation and append-only-trigger probes; versioned M5 databases still stop before M6 until the explicit staged migration supplies its evidence. The independent Worker, durable stream storage, Gateway replay, and quota enforcement are operational on this branch; audit and recovery workflows remain later M6 tasks.
...
- `PrivateRunAdmissionService` composes `PrivateRunRepository`, `RunSnapshotRepository`, and `PrivateRunJobRepository` inside one caller-owned transaction. New Runs are inserted before their job to satisfy the exact-scope job foreign key, then `runs.job_id` is attached before quota reservation and the audit hook; any hook or database failure rolls back Run, snapshots, job, attachment, and quota ledger together. Gateway installs the persistent quota port; the default no-op remains only for isolated compatibility tests until the audit task installs its second hook. Job IDs remain server authority and are not added to the public Run response.
