...
- `app.gateway.routers.project_automations` exposes the strict project API at `/api/projects/{project_id}/automations`. Its readiness route is mounted separately so migration/readiness state remains observable before project-open, while every data route uses `AutomationRoute`, server-issued `PrivateWorkContext`, and the project cutover guard. Request/query/header models reject client authority; public responses omit owner, membership, lease, idempotency hash, occurrence key, and raw error detail. Manual trigger requires a UUID idempotency header, calls the same atomic occurrence/Run/job admission used by Scheduler, and remains available when automatic polling is disabled; its dispatcher dependency first requires the composed operational audit sink and returns the stable Automation 503 before admission when that sink is missing. The legacy scheduled-task router freezes all mutations as soon as the expand table exists and closes all routes after cutover. Its lifespan-bound compatibility adapter exposes reads only: expand queries use the authenticated retained `user_id`, run history rechecks owner through the parent task, and final-schema pre-cutover projections use exact `owner_user_id` while failing closed if one owner spans more than one project; it has no mutation, lease, claim, or dispatch methods and is torn down before the database engine.
...
- M5 completed its Task 18 full-stack gates and independent closure review on 2026-07-16. M6 Tasks 4–15 now provide durable generic jobs, an independent Worker, Worker-only private execution, atomic Automation occurrence/Run/job admission, an independent Scheduler, PostgreSQL durable stream writer/reader/terminal invariants, Gateway SSE reconnect, frontend cursor/dedupe, enforced project quotas, privacy-safe append-only audit sinks for every currently existing governance/run/job caller, the project Admin usage/audit console, and the system-admin operations console. Task 13 closed and passed four repair reviews on 2026-07-17: process actors are bound to the issuing `AuditService`, Gateway governance mutation dependencies fail closed with an Automation-specific stable error envelope for both definition service and manual-trigger dispatcher paths, Automation pause/resume audit in the definition transaction, safe requeue signing is inseparable from the exact locked predecessor/new successor transaction and its capability is consumed once, `PrivateRunRepository` return annotations match the runtime contracts, and production Worker dead settlement emits exactly one `job.dead` plus one `run.terminal`. Same-role membership updates are true no-ops for both version and audit. Task 14 adds strict Admin-only project usage reads, optimistic quota tightening, and descending audit pagination; non-Admin/cross-project access is public 404, and quota policy plus threshold ledger changes roll back when their same-transaction audit append fails. Task 15 exposes only allowlisted platform readiness/counts/aggregate usage, project IDs and status, public job state, and public audit metadata. Every route locks and revalidates the caller's current `system_admin` database role; data routes require M6 Gateway readiness, while the overview reads readiness first and returns `data_status="unavailable"` with null counts/usage when it is closed, without querying possibly absent M6 aggregate tables. System safe requeue accepts only exact project+dead coordinates, delegates to `JobRepository.requeue_safe_system()`, and appends its one-shot `SystemJobAuditSink` event in the same transaction; owner/run/thread/payload/exception authority never enters the HTTP contract. Tasks 16–17 still own backup/restore/purge modules plus their caller wiring; no recovery placeholder module was created early. Backup/restore, remaining orchestration, and final M6 closure are still open. Follow `docs/operations/m5-automation-migration.md` for the maintenance-window cutover; never log private titles/prompts, owner maps, or full resource identifiers.
- M6 Task 16's `app.recovery.archive` is an operator-only archive primitive, not a restore service. `make backup-db ARGS="--output /secure/backups"` reads `DATABASE_URL`, the independent base64 32-byte `DEER_FLOW_BACKUP_KEY`, and the existing audit HMAC environment. It holds one cancellation-safe read-only repeatable-read exporter transaction through dump completion; actual Alembic revision, a privacy-safe digest of `pg_control_system().system_identifier` plus database OID, event high-watermark, proven contiguous tombstone prefix, and table count all come from that snapshot, which is passed to fixed password-free `pg_dump --format=custom --no-owner --no-acl --snapshot=...` argv. The backup database role therefore requires `pg_control_system()` access and otherwise fails closed. Every external-root ancestor is opened fd-relatively with `O_DIRECTORY|O_NOFOLLOW`; the retained final fd owns `0600` passfile and staging/publication operations. `pg_dump` receives only an inherited `/dev/fd/N` passfile descriptor plus `pass_fds`, never a mutable absolute credential path. Passfile ownership begins immediately after parent pinning: open failure closes that parent, while post-open control/write/fsync/lseek failure retains the name/inode/file fd/pinned directory and reuses the same identity-safe release until deletion or confirmed absence survives directory fsync. Transient stat/unlink/fsync failure is retryable; control characters remain rejected. Writer enter/chunk thread tasks also settle through cancellation before abort/close can run. Archive-format version is separate from schema revision; the manifest records actual pg_dump version and non-empty plaintext/ciphertext counts. Per-archive HKDF keys plus deterministic counter nonces prevent cross-archive nonce reuse; AAD binds archive ID, actual revision, authoritative source identity, and chunk index. The writer and reader share a 16 MiB manifest limit and a documented maximum of 65,536 chunks of at most 1 MiB (64 GiB plaintext). Reader authentication uses two ciphertext passes plus a 0600 bounded plaintext spool, yielding nothing until the complete archive authenticates. Backup-key separation follows Auth precedence: explicit `AUTH_JWT_SECRET`, otherwise the existing `DEER_FLOW_HOME/.jwt_secret` read without creation/rotation; missing or unsafe Auth material fails closed. `TrustedOperationAuditSink.backup_created()` is mandatory, and its successful transaction commit is the durable operation commit point: pre-commit failure removes the archive, while later cancellation or engine disposal preserves the audited archive. Command output is restricted to public archive ID/revision/count/checksum; it excludes URLs, locations, keys, plaintext, and subprocess output. Restore, recovery journal, retention purge, and drills remain Task 17 work, and must consume only this module's public facade rather than extending the large archive implementation file.
- M6 reliability contracts are in progress. `config.yaml` version 23 adds restart-required `worker`, `quotas`, and non-secret `recovery` policy. `app.reliability` owns the public-safe error mapping, immutable readiness result, and `ReliabilityCutoverGuard`: queue/Gateway/Worker paths re-read complete M4, M5, and M6 markers plus the M6 final revision on every boundary, while legacy in-Gateway execution closes once the M6 marker completes. Fresh empty databases write a truthful empty-domain M6 marker only after final relation and append-only-trigger probes; versioned M5 databases still stop before M6 until the explicit staged migration supplies its evidence. The independent Worker, durable stream storage, Gateway replay, quota enforcement, and current-domain audit wiring are operational on this branch; recovery workflows remain later M6 tasks.
- M6 Task 17 supersedes the pre-Task17 recovery boundary described above. `app.recovery.journal` writes a separate operator-owned external JSONL journal with an independent base64 32-byte `DEER_FLOW_RECOVERY_JOURNAL_KEY`; each tombstone is AEAD encrypted, previous-digest chained, monotonically sequenced under a cross-process lock, and fsynced before its PostgreSQL physical-purge transaction may complete. Paths must be outside the repository, non-symlink, and permissioned `0700`/`0600`; the journal key must differ from backup, Auth, audit, credential, and database-password material. PostgreSQL retains only sequence, ciphertext digest, resource kind, HMAC reference, status, and timestamps. File purge re-locks the exact project/membership/file and its logical-deletion-plus-30-day timestamp; project purge re-locks the exact expired `pending_deletion` project. Because there is no global account-deletion lifecycle row, recovery-only account purge locks all memberships in stable order, requires zero active memberships and every retention window expired, verifies the exact retained project set, deletes only that owner's private rows in those projects, and retains `users`, governance, jobs, audit, and recovery evidence.
- `make restore-db ARGS="--archive ... --target-url ... --journal ... --execute"` authenticates the complete archive before target creation, accepts only a nonexistent `deerflow_restore_<pid>_<32hex>` database distinct from current authority, runs fixed `pg_restore --exit-on-error --no-owner --no-acl`, replays every authenticated journal record after the archive high-watermark without gaps, runs M1–M6 schema/revision/table/constraint/source-identity/isolation/tombstone probes, and writes `restore_proofs` plus `restore.completed` atomically. Failure/cancellation removes only a target created by that invocation. It never overwrites an existing database, changes `DATABASE_URL`, starts services, or cuts traffic; proof verification and switching remain separate operator actions. `make drill-restore ARGS="--archive ... --journal ..."` generates one random restore database, runs the same workflow, records `recovery_drill.completed`, and always drops only that generated database. CLI output contains only archive ID, revision, table count, status, truncated checksum, and proof ID.
...
**M1–M5 PostgreSQL 发布门禁**：`tests/integration/test_m1_postgres_cutover.py` 串联
...
`tests/integration/test_m4_private_work_postgres.py` 与
`tests/integration/test_m4_private_work_migration_postgres.py` 验证 private-work scope、运行链、
文件/Memory/connection authority 和 staged migration；
`tests/integration/test_m5_project_automation_postgres.py` 与
`tests/integration/test_m5_automation_migration_postgres.py` 验证 Automation scope、并发 claim、
crash replay 边界、fresh/legacy migration、fail-before-DDL 和 cutover。
八个文件只复用 `POSTGRES_TEST_URL` 创建随机 `deerflow_test_*` 数据库，连接或清理失败必须 fail。
缺变量只能让本地日常测试明确 skip；M1–M5 release evidence 必须提供该变量并保持 0 skip。CI 入口为
`.github/workflows/project-foundation-postgres-tests.yml`，CI 缺少 `POSTGRES_TEST_URL` 时必须在
pytest 前硬失败；本地 exact gate：
...
- `migrations/versions/0014_project_reliability_expand.py` — M6 in-progress expand revision for jobs/Workers, quota, audit, recovery controls, nullable Run/occurrence job authority, deletion-stable Thread event high-watermarks, and the partial unique Run stream terminal index
...
M4 private-work migration、M5 project Automation 与 M5 Automation migration 八个真实 PostgreSQL
integration 文件。缺少 `POSTGRES_TEST_URL` 只能在本地日常测试中明确 skip；M1–M5 release evidence
必须提供该变量并保持 0 skip，CI 缺少该变量时在 pytest 前硬失败。
