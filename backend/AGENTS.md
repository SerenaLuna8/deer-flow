...
- M5 Automation completion and restart recovery are owned by `AutomationReconciler`, not callback metadata or the legacy scheduler repositories. The worker invokes its terminal completion hook exactly once only after private runtime/file-authority cleanup has established the final authoritative M4 Run status. Completion re-derives project+owner scope, locks project/membership -> definition -> occurrence -> Run, and increments the parent count only after the first terminal occurrence CAS. Automation restart reconciliation is independent of automatic polling: for a production `AppConfig` with final M5 cutover open it always runs before generic M4 orphan recovery, including `scheduler.enabled=false`, so crashed manual occurrences cannot remain active. With `scheduler.enabled=true`, `GATEWAY_WORKERS=1` is the quick topology guard and `AutomationSchedulerOwnership` is the authoritative cross-process singleton: it holds fixed PostgreSQL session advisory lock `0x0DEE_12F1_0A55_0007` on one dedicated connection for the Gateway runtime lifetime. Ownership is acquired before reconciliation, records the original PostgreSQL backend PID, and verifies before each poll—using a read-only `pg_locks` query on the same connection, never another `pg_try_advisory_lock`—that both the physical session and lock still match. Disconnect, automatic reconnect/PID drift, restart, or missing lock makes ownership permanently `ownership_lost`; the poll loop exits before another reserve/claim/dispatch and cannot silently reacquire, while a second Gateway can take over after PostgreSQL releases the old session lock. Readiness reports this scheduler status separately without closing otherwise-ready manual/project APIs. Lock contention, ownership loss during startup, or reconciliation/database failure fails closed before unsafe mutation. An expired `launching` occurrence is requeued only when no deterministic private Run exists, while an admitted active Run is interrupted and never replayed. Disabled mode takes no ownership connection or poll monitor but retains manual APIs and startup reconciliation; unsupported multi-worker mode remains initialized but does not reconcile or poll. Shutdown cancels the scheduler before releasing ownership and closing runtime dependencies.
...
- `app.gateway.routers.project_automations` exposes the strict project API at `/api/projects/{project_id}/automations`. Its readiness route is mounted separately so migration/readiness state remains observable before project-open, while every data route uses `AutomationRoute`, server-issued `PrivateWorkContext`, and the project cutover guard. Request/query/header models reject client authority; public responses omit owner, membership, lease, idempotency hash, occurrence key, and raw error detail. Manual trigger requires a UUID idempotency header, reserves and claims that exact scoped occurrence before dispatch, and remains available when automatic polling is disabled. The legacy scheduled-task router freezes all mutations as soon as the expand table exists and closes all routes after cutover. Its lifespan-bound compatibility adapter exposes reads only: expand queries use the authenticated retained `user_id`, run history rechecks owner through the parent task, and final-schema pre-cutover projections use exact `owner_user_id` while failing closed if one owner spans more than one project; it has no mutation, lease, claim, or dispatch methods and is torn down before the database engine.
- M5 scheduler configuration has exactly five restart-required fields: `enabled`, `poll_interval_seconds`, `lease_seconds`, `max_concurrent_runs`, and `min_once_delay_seconds`. The lease must exceed the positive poll interval; concurrency must be positive; once delay may be zero but not negative and applies to creation plus semantic schedule changes. Update payloads that repeat the persisted normalized `schedule_spec`/`timezone` are ordinary non-schedule edits: they do not recompute `next_run_at` or apply the minimum-delay check; a real schedule difference is still validated before queued occurrences or definitions are mutated. Standard `$VARIABLE` config values are resolved before validation. Gateway installs one `ScheduledTaskService` under both the legacy compatibility name and `automation_scheduler`, records only that service's one poll task as `automation_scheduler_task`, and clears it after awaited cancellation. Marker-not-ready, disabled, missing ownership, and unsupported worker topology leave the poll task unset while keeping manual dispatcher and readiness dependencies available.
- M5 completed its Task 18 full-stack gates and independent closure review on 2026-07-16. Its operational boundary remains one Gateway with `scheduler.enabled` controlling only automatic polling; project APIs and manual trigger remain available when polling is disabled. Every occurrence is reserved before private-run admission, and restart recovery never replays an admitted run. Independent Workers, durable SSE, generic jobs/retries, quotas, audit, and general backup/restore remain M6 responsibilities. Follow `docs/operations/m5-automation-migration.md` for the maintenance-window cutover; never log private titles/prompts, owner maps, or full resource identifiers.
- M6 reliability contracts are in progress. `config.yaml` version 23 adds restart-required `worker`, `quotas`, and non-secret `recovery` policy. `app.reliability` owns the public-safe error mapping, immutable readiness result, and `ReliabilityCutoverGuard`: queue/Gateway/Worker paths re-read complete M4, M5, and M6 markers plus the M6 final revision on every boundary, while legacy in-Gateway execution closes once the M6 marker completes. Fresh empty databases write a truthful empty-domain M6 marker only after final relation and append-only-trigger probes; versioned M5 databases still stop before M6 until the explicit staged migration supplies its evidence. This contract does not yet mean the independent Worker, quotas, stream replay, audit sink, or recovery workflows are operational; those remain later M6 tasks.
...
- `scheduler` - Embedded project Automation poller (`enabled`, `poll_interval_seconds`, `lease_seconds`, `max_concurrent_runs`, `min_once_delay_seconds`); restart-required, single-Gateway only in M5
