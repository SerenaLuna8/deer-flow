...
- M5 completed its Task 18 full-stack gates and independent closure review on 2026-07-16. M6 Tasks 4–11 now provide durable generic jobs, an independent Worker, Worker-only private execution, atomic Automation occurrence/Run/job admission, an independent Scheduler, PostgreSQL durable stream writer/reader/terminal invariants, Gateway SSE reconnect, frontend cursor/dedupe, and enforced project quotas. The full audit sink, backup/restore, remaining orchestration, and final M6 closure are still open. Follow `docs/operations/m5-automation-migration.md` for the maintenance-window cutover; never log private titles/prompts, owner maps, or full resource identifiers.
