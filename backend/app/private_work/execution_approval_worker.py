"""Worker-owned, lease-bound Host Execution Approval port.

The Worker is the only process that stages a frozen plan, claims a one-shot
launch, authorizes the final spawn, and persists the durable receipt.  Every
method revalidates the exact Job lease inside its own transaction and keeps
the established Project -> Membership -> Thread -> Job -> Run -> Attempt ->
Approval -> Receipt lock order.
"""
