...
class SkillBuilderRunAdmissionService(SkillBuilderRunAdmissionPort):
    ...
    async def admit_in_session(
        ...
        *,
        ...
        payload = self._run_input_payload(
            ...
            authoring_kind=design.session_kind,
            base_version_number=design.base_version_number,
    ...
    def _run_input_payload(
        ...
        *,
        ...
        if (
            ...
            or authoring_kind not in {"create", "revise"}
            or (authoring_kind == "revise" and (type(base_version_number) is not int or base_version_number < 1))
            or (authoring_kind == "create" and base_version_number is not None)
        ):
            raise AssetValidationFailed(request_id)
        authoring: dict[str, object] = {"kind": authoring_kind}
        if authoring_kind == "revise":
            authoring["target_slug"] = request.skill_slug
            authoring["base_version_number"] = base_version_number
...
__all__ = ["SkillBuilderRunAdmissionService"]
