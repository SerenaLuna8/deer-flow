...
def exact_model_ref(value: object) -> str | None:
    """Return one canonical System Model UUID string or ``None``."""
...
def resolve_model_ref(config: Any, model_ref: str) -> ModelConfig | Any | None:
    """Resolve ``default`` or one exact stable System Model UUID."""
...
class ExactModelRefResolver:
    """Exact-only fallback for isolated repository tests."""
