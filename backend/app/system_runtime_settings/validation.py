...
# The pre-release policy history (schemas v2-v7) was consolidated into this
# single v1 baseline before launch. Stored rows and Run snapshots therefore
# carry exactly one Runtime Policy schema; any other number fails closed.
# This is distinct from the PostgreSQL DDL "Schema V1" marker.
...
def canonical_policy_payload_for_schema(
    ...
    *,
    ...
    """Canonicalize a stored payload against the one supported schema.

    The pre-release version history was consolidated: every stored row and Run
    snapshot must declare Runtime Policy schema v1, and any other declared
    version fails closed instead of being repaired or upgraded in place.
    """

    if type(schema_version) is not int or schema_version != RUNTIME_POLICY_SCHEMA_VERSION:
        raise RuntimePolicyInvalid
...
def decode_policy_value_for_schema(
    ...
    *,
    ...
    try:
        return parse_policy_value(RuntimePolicySection(section), canonical.value)
    except RuntimePolicyInvalid:
        raise
