...
async def _seed_v12_conversation(
    factory: async_sessionmaker[AsyncSession],
    seed: _LifecycleSeed,
) -> tuple[str, str]:
    """Seed only columns present in the historical v12 schema."""
    old_thread_id = uuid.uuid4().hex
    conversation_id = uuid.uuid4().hex
    now = datetime.now(UTC)
    async with factory() as session, session.begin():
...
async def _pre_v14_thread_agent_asset_id(
    session: AsyncSession,
    *,
    thread_id: str,
) -> uuid.UUID | None:
    return await session.scalar(
        text("SELECT agent_asset_id FROM threads_meta WHERE thread_id = :thread_id"),
        {"thread_id": thread_id},
    )
...
@pytest.mark.postgres
@pytest.mark.asyncio
async def test_v13_migration_releases_legacy_tombstone_without_rekeying_identity(
    ...
    async with temporary_postgres_database(postgres_admin_url) as database_url:
        ...
        await asyncio.to_thread(_upgrade_to, database_url, "full_schema_v12")

        v12_engine = create_async_engine(database_url, poolclass=NullPool)
        try:
            factory = async_sessionmaker(v12_engine, expire_on_commit=False)
            seed = await _seed_lifecycle(factory)
            ordinary_connection_id = await _seed_ordinary_same_owner_connection(
                factory,
                seed,
            )
            old_thread_id, old_conversation_id = await _seed_v12_conversation(
                factory,
                seed,
            )
            deleted_at = datetime.now(UTC)
            async with v12_engine.begin() as connection:
                await connection.execute(
                    text(
                        """UPDATE project_channel_group_bindings
                              SET status = 'disabled', deleted_at = :deleted_at
                            WHERE id = :binding_id"""
                    ),
                    {
                        "binding_id": seed.binding_id,
            ...
            await v12_engine.dispose()

        await asyncio.to_thread(_upgrade_to, database_url, "full_schema_v13")

        v13_engine = create_async_engine(database_url, poolclass=NullPool)
        try:
            async with v13_engine.connect() as connection:
                marker = await connection.scalar(text("SELECT version_num FROM alembic_version"))
            factory = async_sessionmaker(v13_engine, expire_on_commit=False)
            async with factory() as session:
                binding = await session.get(ProjectChannelGroupBindingRow, seed.binding_id)
                principal = await session.get(ChannelExternalPrincipalRow, seed.principal_id)
                membership = await session.get(ProjectMembershipRow, seed.guest_membership_id)
                ...
                old_thread_agent_id = await _pre_v14_thread_agent_asset_id(
                    session,
                    thread_id=old_thread_id,
                )
                ordinary_connection = await session.get(
            ...
            assert marker == "full_schema_v13"
            assert binding is not None and binding.id == seed.binding_id
            assert binding.status == "disabled"
            assert binding.deleted_at == deleted_at
            assert binding.agent_asset_id is None
            assert binding.agent_scope is None
            ...
            assert old_thread_agent_id == seed.old_agent_id
            assert old_conversation is None
            assert ordinary_connection is not None
            assert ordinary_connection.status == "connected"
            assert ordinary_connection.frozen_at is None
            assert ordinary_connection.metadata_json == {}
        finally:
            await v13_engine.dispose()


@pytest.mark.postgres
@pytest.mark.asyncio
async def test_v13_migration_normalizes_legacy_live_disabled_routing(
    postgres_admin_url: str,
) -> None:
    async with temporary_postgres_database(postgres_admin_url) as database_url:
        await _execute_sql_batch(database_url, _baseline_sql())
        ...
        try:
            factory = async_sessionmaker(v12_engine, expire_on_commit=False)
            changed = await _seed_lifecycle(factory)
            changed_thread_id, changed_conversation_id = await _seed_v12_conversation(
                factory,
                changed,
            )
            unchanged = await _seed_lifecycle(factory)
            unchanged_thread_id, unchanged_conversation_id = await _seed_v12_conversation(
                factory,
                unchanged,
            )
            changed_at = datetime.now(UTC)
            async with v12_engine.begin() as connection:
                await connection.execute(
                    text(
                        """UPDATE project_channel_group_bindings
                              SET status = 'disabled',
                                  agent_asset_id = :new_agent_id,
                                  updated_at = :changed_at
                            WHERE id = :binding_id"""
                    ),
                    {
                        "binding_id": changed.binding_id,
                        "new_agent_id": changed.new_agent_id,
        ...
        try:
            factory = async_sessionmaker(v13_engine, expire_on_commit=False)
            async with factory() as session:
                changed_binding = await session.get(
                    ProjectChannelGroupBindingRow,
                    changed.binding_id,
                )
                changed_principal = await session.get(
                    ChannelExternalPrincipalRow,
                    changed.principal_id,
                )
                changed_connection = await session.get(
                    ChannelConnectionRow,
                    changed.connection_id,
                )
                changed_thread_agent_id = await _pre_v14_thread_agent_asset_id(
                    session,
                    thread_id=changed_thread_id,
                )
                changed_conversation = await session.get(
                    ChannelConversationRow,
                    changed_conversation_id,
                )
                unchanged_principal = await session.get(
                    ChannelExternalPrincipalRow,
                    unchanged.principal_id,
                )
                unchanged_connection = await session.get(
                    ChannelConnectionRow,
                    unchanged.connection_id,
                )
                unchanged_thread_agent_id = await _pre_v14_thread_agent_asset_id(
                    session,
                    thread_id=unchanged_thread_id,
                )
                unchanged_conversation = await session.get(
                    ChannelConversationRow,
                    unchanged_conversation_id,
                )

            assert changed_binding is not None
            assert changed_binding.status == "disabled"
            assert changed_binding.deleted_at is None
            assert changed_binding.agent_asset_id == changed.new_agent_id
            assert changed_principal is not None
            assert changed_principal.status == "frozen"
            assert changed_connection is not None
            assert changed_connection.status == "frozen"
            assert changed_connection.frozen_at is not None
            assert changed_connection.frozen_at >= changed_at
            assert changed_connection.metadata_json == {
                "group_binding_id": str(changed.binding_id),
                "agent_asset_id": str(changed.new_agent_id),
                "agent_scope": "project",
            }
            assert changed_thread_agent_id == changed.old_agent_id
            assert changed_conversation is None

            assert unchanged_principal is not None
            assert unchanged_principal.status == "frozen"
            assert unchanged_connection is not None
            assert unchanged_connection.status == "frozen"
            assert unchanged_connection.frozen_at is not None
            assert unchanged_connection.frozen_at >= changed_at
            assert unchanged_connection.metadata_json == {
                "group_binding_id": str(unchanged.binding_id),
                "agent_asset_id": str(unchanged.old_agent_id),
                "agent_scope": "project",
            }
            assert unchanged_thread_agent_id == unchanged.old_agent_id
            assert unchanged_conversation is not None
            assert unchanged_conversation.thread_id == unchanged_thread_id

            changed_context = PrivateWorkContext.from_project(
                ProjectContext(
                    user_id=changed.guest_user_id,
                    project_id=changed.context.project_id,
                    membership_id=changed.guest_membership_id,
                    role=ProjectRole.CHANNEL_GUEST,
                    capabilities=capabilities_for(ProjectRole.CHANNEL_GUEST),
                    membership_version=1,
                    request_id="legacy-disabled-stale-inbound",
                )
            )
            server_context = PrivateRunAdmissionServerContext(
                inbound_authority=PrivateRunInboundAuthority(
                    ...
                    provider="lark",
                    external_account_id=changed.external_account_ref,
                    workspace_id=changed.external_group_ref,
                    external_conversation_id=changed.external_group_ref,
                    external_topic_id=None,
                    channel_instance_id=str(changed.channel_instance_id),
                ),
                inbound_delivery=PrivateRunInboundDelivery(
                    "legacy-disabled-stale-delivery",
                ),
            )
            async with factory() as session:
                with pytest.raises(PrivateWorkNotFound):
