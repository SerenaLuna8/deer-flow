...
from app.shared_assets.crypto import decrypt_credential_payload, encrypt_credential_payload
...
def test_project_mcp_policy_accepts_query_credential_against_secret_free_base_url() -> None:
    assert (
        validate_project_mcp_definition(
            transport="http",
            url=_BASE_ENDPOINT,
            ...
            headers={},
            oauth={},
            credential_slot_schemas=({"query": ("key",)},),
...
def test_project_mcp_policy_rejects_ambiguous_or_unsafe_query_credential_names(
    url: str,
    ...
    with pytest.raises(McpDefinitionPolicyError):
        validate_project_mcp_definition(
            ...
            headers={},
            oauth={},
            credential_slot_schemas=schemas,
...
def test_project_mcp_service_accepts_query_slot_without_persisting_secret() -> None:
    ...
    assert normalized.credential_slots[0].payload_schema == {"query": ("key",)}


def test_project_credential_query_payload_encrypts_and_exposes_only_schema() -> None:
    context = _context()
    version_id = uuid.uuid4()
    secret = "query-secret-with symbols/+"
    payload = {"query": {"key": secret}}
    keyring = CredentialKeyring(active_key_id="test", _keys={"test": b"q" * 32})

    schema = CredentialService._payload_schema(context, payload)
...
def test_run_admission_and_worker_runtime_accept_query_credential_material() -> None:
    version_id = uuid.uuid4()
...
def test_worker_runtime_rejects_unknown_project_credential_material_group() -> None:
    snapshot = SimpleNamespace(scope=AssetScope.PROJECT)
    with pytest.raises(McpDefinitionPolicyError):
        PrivateAgentRuntime._validate_project_mcp_material(
            snapshot,
