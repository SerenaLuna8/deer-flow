...
async def _seed_memory_model(
    ...
    *,
    ...
    logical_name = f"memory-freeze-{model_id.hex}"
    await session.execute(
        sa.text(
            ...
            VALUES (:id,:name,'Memory freeze model','Memory freeze PG model',
                    'active',NULL,1,0,:owner,:owner)"""
        ),
        {"id": model_id, "name": logical_name, "owner": actor_user_id},
    )
    await session.execute(
        sa.text(
            ...
            (id,model_config_id,version_number,provider_adapter,provider_model,
             ...
             created_by_user_id)
            VALUES (:id,:model,1,'codex_cli','memory-freeze','{}'::jsonb,
    ...
    return logical_name, version_id
