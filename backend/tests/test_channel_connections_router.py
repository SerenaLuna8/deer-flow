...
@pytest_asyncio.fixture(params=("0007_project_shared_assets", "0008_project_private_work_expand"))
async def _postgres_database(postgres_database_url, request):
    global _DATABASE_URL
    bootstrap_engine = create_async_engine(postgres_database_url, poolclass=NullPool)
    await asyncio.to_thread(
        command.upgrade,
        _get_alembic_config(bootstrap_engine),
        request.param,
    )
    await bootstrap_engine.dispose()
    _DATABASE_URL = postgres_database_url
...
async def _make_repo(_tmp_path):
    ...
    repo = LegacyChannelConnectionRepository(async_sessionmaker(engine, expire_on_commit=False))
...
def _make_app(
    ...
    *,
    ...
    app.state.legacy_channel_connection_repo = repo
