"""Tests for the shared host-to-virtual output masking pattern."""
...
def test_helper_reproduces_pre_extraction_expressions(base: str) -> None:
...
def test_separator_agnostic_is_the_only_difference_between_modes() -> None:
    windows_base = "C:\\host\\skills"
    posix_spelling = "C:/host/skills/file.md"
    assert build_output_mask_pattern(
        windows_base,
        separator_agnostic=True,
    ).search(posix_spelling)
    ...
    assert (
        ...
        == build_output_mask_pattern(posix_base).pattern
    )


def test_boundary_rejects_prefix_siblings_and_accepts_real_segments() -> None:
...
def test_local_sandbox_reverse_patterns_use_shared_helper(
    tmp_path: Path,
) -> None:
...
def test_tools_mask_patterns_use_shared_helper(tmp_path: Path) -> None:
