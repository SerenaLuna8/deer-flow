...
2. **运行安装向导（推荐）**
   ...
   登录后的默认入口是 `/workspace/projects` 项目工作台，可搜索、创建、置顶和按能力编辑
   项目；静态演示构建仍进入原有示例对话，且不会暴露会请求项目 API 的入口。项目卡只展示
   API 的公开字段。进入项目时，前端会通过成员可见的项目列表精确匹配 slug，再用返回的
   UUID 调用 enter，绝不会把 slug 传给只接受 UUID 的详情或 enter 接口。
