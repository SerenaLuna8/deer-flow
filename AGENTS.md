...
邮件；成员退出/移除和项目删除只记录 30 天窗口，不物理清除私有或项目数据。M4 已完成
Thread、run、file 以及 Memory 存储/队列/prompt 的项目与 owner 双重隔离；Memory 项目路由、
legacy cutover guard 和 automation 仍由后续 Task 完成，因此当前仍不能作为完整多用户 SaaS 发布。
