...
Not directly executable: import as `detectors.blocking_io_static` or
...
from detectors.repo_root import resolve_repo_root
...
class BlockingIOStaticVisitor(ast.NodeVisitor):
    ...
    def current_sync_http_clients(self) -> dict[str, str]:
        return (
            self.sync_http_client_stack[-1]
            if self.sync_http_client_stack
            else self.module_sync_http_clients
        )
    ...
    def _visit_function(
        ...
        self.path_like_name_stack.append(
            set(_path_like_argument_names(node.args, self._canonical_name))
        )
        self.generic_visit(node)
    ...
    def _record_call_ref(
        self, node: ast.Call, call_name: str, current: _FunctionContext
    ) -> None:
        if current.qualname == "<module>":
            return
        if isinstance(node.func, ast.Name):
            self.call_refs[current.qualname].append(
                _CallRef(node.func.id, current.class_name, self_method=False)
            )
            return
        ...
        if receiver in {"self", "cls"}:
            self.call_refs[current.qualname].append(
                _CallRef(node.func.attr, current.class_name, self_method=True)
            )
            return
        ...
        if "." not in call_name:
            self.call_refs[current.qualname].append(
                _CallRef(call_name, current.class_name, self_method=False)
            )

    def _record_blocking_candidate(
        self, node: ast.Call, call_name: str, current: _FunctionContext
    ) -> None:
    ...
    def _blocking_rule(self, node: ast.Call, call_name: str) -> _BlockingRule | None:
        ...
        if chained_client_symbol is not None:
            return _BlockingRule(
                "BLOCKING_HTTP_IO", "HTTP_REQUEST", chained_client_symbol
            )
        ...
        if leaf_name in PATH_METHOD_NAMES | AMBIGUOUS_PATH_METHOD_NAMES:
            if self._is_path_method_call(node):
                return _BlockingRule(
                    "BLOCKING_FILE_IO", _path_method_operation(leaf_name), call_name
                )
        if call_name in BLOCKING_OS_FILE_NAMES:
            return _BlockingRule(
                "BLOCKING_FILE_IO", OS_FILE_OPERATIONS[call_name], call_name
            )
        ...
        if call_name in BLOCKING_SHUTIL_NAMES:
            return _BlockingRule(
                "BLOCKING_FILE_IO", SHUTIL_OPERATIONS[call_name], call_name
            )
        return None
    ...
    def _is_path_method_call(self, node: ast.Call) -> bool:
        if not isinstance(node.func, ast.Attribute):
            return False
        if (
            node.func.attr in AMBIGUOUS_PATH_METHOD_NAMES
    ...
    def _record_path_like_annotation(
        self, annotation: ast.AST, targets: Iterable[ast.AST]
    ) -> None:
        if not self.path_like_name_stack or not _is_path_annotation(
            annotation, self._canonical_name
        ):
            return
        self.current_path_like_names.update(
            name for target in targets for name in _iter_assigned_names(target)
        )

    def _record_sync_http_client_targets(
        self, value: ast.AST, targets: Iterable[ast.AST]
    ) -> None:
...
def _looks_like_path_receiver_name(receiver_name: str | None) -> bool:
    ...
    return (
        leaf
        in {"path", "file_path", "dir_path", "target", "dest", "destination", "source"}
        or leaf.endswith(("_path", "_dir", "_file", "_root"))
        or "path" in leaf
    )


def _is_path_annotation(
    annotation: ast.AST | None, canonical_name: Callable[[str | None], str | None]
) -> bool:
    if annotation is None:
        return False
    if isinstance(annotation, ast.BinOp) and isinstance(annotation.op, ast.BitOr):
        return _is_path_annotation(
            annotation.left, canonical_name
        ) or _is_path_annotation(annotation.right, canonical_name)
...
def _reachable_functions(
    visitor: BlockingIOStaticVisitor, roots: Iterable[str]
) -> set[str]:
    reachable = set(roots)
    ...
    while changed:
        ...
        for class_name, bases in visitor.class_bases.items():
            if class_name in middleware_classes:
                continue
            if any(
                _is_agent_middleware_base(base, middleware_classes) for base in bases
            ):
...
def _sync_only_agent_middleware_entrypoints(
...
def _finalize_findings(
    visitor: BlockingIOStaticVisitor,
) -> list[BlockingIOStaticFinding]:
    reachable = _async_reachable_functions(visitor)
    middleware_reachable = _reachable_functions(
        visitor, _sync_only_agent_middleware_entrypoints(visitor)
    )
    event_loop_exposures = _event_loop_exposures(
        visitor, reachable, middleware_reachable
    )
...
def scan_file(
    ...
    return scan_source(
        path.read_text(encoding="utf-8"), relative_to_repo(path, repo_root)
    )
...
def write_json_report(
    findings: Sequence[BlockingIOStaticFinding], output_path: Path
) -> None:
...
def _format_counter(
    ...
    *,
    ...
    if order is None:
        ...
        extras = sorted(
            (item for item in counter.items() if item[0] not in ordered_names),
...
def format_summary(
    findings: Sequence[BlockingIOStaticFinding], *, output_path: Path | None = None
) -> str:
    if not findings:
        lines = [
        ...
        lines = [
            ...
            "",
            *_format_counter(
                "By category:", Counter(finding.category for finding in findings)
            ),
            "",
            *_format_counter(
                "By priority:",
                Counter(finding.priority for finding in findings),
                order=("HIGH", "MEDIUM", "LOW"),
            ),
            "",
            *_format_counter(
                "By operation:", Counter(finding.operation for finding in findings)
            ),
            "",
            *_format_counter(
                "By event-loop exposure:",
                Counter(finding.exposure for finding in findings),
            ),
            "",
            *_format_counter(
                "By scan root:",
                Counter(_scan_root(finding.path) for finding in findings),
            ),
            "",
            *_format_counter(
                "Top files:", Counter(finding.path for finding in findings), limit=10
            ),
        ]

    if output_path is not None:
        lines.extend(
            ["", f"Full JSON report: {relative_to_repo(output_path.resolve())}"]
        )
    else:
...
def format_text(findings: Sequence[BlockingIOStaticFinding]) -> str:
    ...
    for finding in findings:
        ...
        lines.append(
            f"  reason: {_finding_reason(finding.operation, finding.exposure)}"
        )
        if finding.code:
