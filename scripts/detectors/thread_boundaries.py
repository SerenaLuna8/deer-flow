...
Not directly executable: import as `detectors.thread_boundaries` or
...
class BoundaryVisitor(ast.NodeVisitor):
    def __init__(
        self, path: Path, relative_path: str, source_lines: Sequence[str]
    ) -> None:
        self.path = path
    ...
    def _check_call(self, node: ast.Call, call_name: str) -> None:
        ...
        if call_name in ASYNC_TOOL_FACTORY_CALLS:
            if any(
                keyword.arg == "coroutine" and not is_none_node(keyword.value)
        ...
        if self.in_async_context and self._is_langchain_invoke(
            node, call_name, method_name="invoke"
        ):
            self._emit(
                node,
                severity="WARN",
        ...
        if not self.in_async_context and self._is_langchain_invoke(
            node, call_name, method_name="ainvoke"
        ):
            self._emit(
                node,
                severity="WARN",
    ...
    def _record_executor_targets(
        self, value: ast.AST, targets: Sequence[ast.AST]
    ) -> None:
    ...
    def _is_langchain_invoke(
        self, node: ast.Call, call_name: str, *, method_name: str
    ) -> bool:
        ...
        return (
            receiver_leaf in LANGCHAIN_INVOKE_RECEIVER_NAMES
            or receiver_leaf.endswith(LANGCHAIN_INVOKE_RECEIVER_SUFFIXES)
        )
    ...
    def _emit(
        self, node: ast.AST, *, severity: str, category: str, symbol: str, message: str
    ) -> None:
...
def filter_findings(
    findings: Iterable[BoundaryFinding], min_severity: str
) -> list[BoundaryFinding]:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.format == "json":
        print(
            json.dumps(
                [finding.to_dict() for finding in findings], indent=2, sort_keys=True
            )
        )
    else:
