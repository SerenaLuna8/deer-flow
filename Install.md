...
- Require an explicit PostgreSQL-only `DATABASE_URL`. Do not read or print its password. `0001_project_saas_baseline` is the immutable baseline and `0002_project_skill_hard_delete` is the current head. If the target database does not exist, ask the user to provide `POSTGRES_ADMIN_URL` and run `make setup-db`; this installs the complete migration chain and performs first-install bootstrap in an empty target. An existing database at the exact verified `0001` ancestor must be upgraded with `make migrate-db`, which applies only pending migrations and performs no catalog, LangGraph, or default-project bootstrap. Then run `make check-db`.
