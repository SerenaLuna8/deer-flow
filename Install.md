...
- Knowledge (RAG knowledge bases) is optional and disabled by default; a missing `knowledge` block in `config.yaml` keeps it off at runtime (`make setup-db` still preflights the Knowledge seed key or its explicit skip switch, see above). Enabling it later requires: an administrator-prepared MinIO S3 API endpoint (`host:port`, not the `:9001` Console) reachable by both Gateway and Worker, a pre-created bucket (default `actweave-knowledge`), the `ACT_WEAVE_KNOWLEDGE_MINIO_ENDPOINT` / `ACT_WEAVE_KNOWLEDGE_MINIO_ACCESS_KEY` / `ACT_WEAVE_KNOWLEDGE_MINIO_SECRET_KEY` environment values, and a restart of Gateway and Worker together. Gateway and Worker verify bucket accessibility at startup and fail fast when it is unreachable; they never create the bucket. Inside Compose the endpoint must be an address both containers can reach (for example `host.docker.internal:9000`), never the container's own `127.0.0.1`.
