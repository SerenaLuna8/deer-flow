...
- Require an explicit PostgreSQL-only `DATABASE_URL`. Do not read or print its password. `0001_project_saas_baseline` is the immutable baseline and `0002_skill_design_builder` is the single current head. If the target database does not exist, ask the user to provide `POSTGRES_ADMIN_URL` and run `make setup-db`; this installs the complete current chain and performs first-install bootstrap in an empty target. A database for the current release must exactly match the `0002` catalog, or exactly match frozen `0001` before an explicit stopped-service `make migrate-db`. Migration performs no catalog, LangGraph, or default-project bootstrap. Then run `make check-db`.
