...
- Require an explicit PostgreSQL-only `DATABASE_URL`. Do not read or print its password. `0001_project_saas_baseline` is the immutable baseline and single current head; it already includes project-Skill package hard deletion and project-local name uniqueness, the four logical Agent documents, and Agent Builder tables. If the target database does not exist, ask the user to provide `POSTGRES_ADMIN_URL` and run `make setup-db`; this installs the current baseline and performs first-install bootstrap in an empty target. A database for the current release must already match that exact baseline catalog. `make migrate-db` is reserved for future forward-only revisions beginning with `0002` and performs no catalog, LangGraph, or default-project bootstrap. Then run `make check-db`.
