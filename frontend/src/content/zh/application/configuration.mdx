...
DeerFlow 必须使用 PostgreSQL 存储 LangGraph 检查点和运行记录、反馈、线程元数据等应用数据。将连接地址放入 `DATABASE_URL`，并通过统一的 `database` 配置引用：
...
`DATABASE_URL` 接受 `postgresql://...` 和 `postgresql+asyncpg://...` 地址。PostgreSQL 驱动已是默认依赖，不需要数据库 extra；独立的 `checkpointer` 配置不再接受。
