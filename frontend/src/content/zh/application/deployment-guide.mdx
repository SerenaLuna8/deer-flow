...
所有 DeerFlow 部署都必须使用 PostgreSQL。驱动已包含在默认依赖中，不需要设置数据库专用的 `UV_EXTRAS`。
...
生产环境应使用独立数据库，并采用标准 PostgreSQL 运维措施。旧 provider 分支在后续清理任务完成前可能仍存在于内部代码，但不能通过公开配置选择。
...
nginx 将项目 REST 与流式请求路由到 Gateway，其余页面请求路由到前端。Gateway 只暴露最终的 account/project 作用域 API。
