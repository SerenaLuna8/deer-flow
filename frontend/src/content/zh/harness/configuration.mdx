...
ActWeave 将基础设施进程配置与数据库治理设置明确分开，使部署可重现、可审计，
并且易于按环境定制。

## 配置文件位置

ActWeave 使用以下优先级顺序解析 `config.yaml`：

1. 显式传递给 `AppConfig.from_file(config_path)` 的路径。
2. `ACT_WEAVE_CONFIG_PATH` 环境变量。
3. 兼容旧部署的 `DEER_FLOW_CONFIG_PATH` 环境变量。
4. 仓库根目录中的 `config.yaml`。

如果两个环境变量同时存在，它们必须解析为同一路径。
...
这是 ActWeave 实现可插拔性而不硬编码类引用的方式。

## 模型 provider 设置

平台管理员在 **Admin → 系统设置 → 模型** 中配置 provider adapter、provider
模型名、能力和有界的 provider 专用 JSON。API key 通过版本化 System Credential
选择，绝不保存在模型设置或 `config.yaml` 中。

每个 Run 都会固定准入时的精确模型配置和 Credential 版本，因此之后的管理变更不会
静默改变已经排队的工作。
