...
<Callout type="info" emoji="📦">
  沙箱是 Agent 进行文件和命令操作的隔离工作区。它让 Fluva
  能够采取真实行动，而不仅仅是对话。
</Callout>
...
<Callout type="warning">
  使用 Docker runtime 时，Worker 与 Docker daemon 必须以相同绝对文件系统视图解析 Run Skill
  挂载源。路径视图不同会禁用这些只读挂载，且没有配置绕过项。
</Callout>
...
命令在 [E2B](https://e2b.dev) 云端的 micro-VM 中执行。每个线程通过 metadata 绑定到一个独立的 e2b 沙箱，因此即使 Gateway 重启或在多进程之间，同一个 Fluva 线程也始终复用同一个 e2b 沙箱。当达到配置的空闲超时时，e2b 会自动回收云端 VM。
...
<Callout type="info">
  因为 e2b 是远程沙箱，Fluva 的虚拟前缀 `/mnt/user-data`
...
可选服务从 `sandbox/provisioner/` 独立运行，负责每个 Sandbox ID 的 Pod 和 Service 生命周期；它不是应用部署入口。
...
| `.fluva-flow/threads/{thread_id}/user-data/` | `/mnt/user-data/`                             | 读写     |
...
`LocalSandboxProvider` 从 Worker 宿主进程解析 `host_path`。如果配置路径对该进程不可见，Fluva 会记录错误日志并忽略该挂载。
