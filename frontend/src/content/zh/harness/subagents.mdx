...
- `max_concurrent_subagents`：单次轮次中最大并行任务调用数（默认：3，执行范围：1–4）

如果 Agent 尝试调用超过限制的子 Agent，中间件会裁剪多余的调用。单个 Run 还受独立的总委派
上限约束（默认：6，配置范围：1–50），避免通过多个轮次无限串行委派。

子任务卡片会显示该次委派实际使用的模型；启用全局 Token 用量展示后，还会显示最新累计
Token。实时事件和刷新后的结构化任务结果使用同一组字段。

## ACP Agent（外部 Agent）

除内置子 Agent 外，ActWeave 还通过 **Agent Connect Protocol (ACP)** 支持委派给外部 Agent。ACP 允许 ActWeave 调用作为独立进程运行的 Agent（包括用 ACP 适配器包装的第三方 CLI 工具）。
...
<Callout type="tip">
  ACP Agent 作为 ActWeave 管理的子进程运行，通过 ACP 协议通信。标准 CLI
