...
加入当前日期提醒；当私有 Run 启用 Memory 时，通过 Worker 签发的 authority 读取该 Run 冻结的召回来源。v2 会在每个模型边界复用本 Run 的精确有序 Fact Revision 快照，并以隐藏的低权限 Human message 注入。这个中间件不准入或更新长期 Memory。

长期写入链路位于中间件链之外：成功 Run 结算时准入 durable Source 和 `memory_extract` Job，再由 Scheduler 与 Worker 完成整理。已移除的内置 `MemoryMiddleware`、进程内 queue/updater 和 summarization Memory hook 不再是运行时扩展点。Pipeline mode 和管理行为参见[记忆系统](/docs/harness/memory)。
