...
12. `ToolCallControl` — 仲裁重复调用与单工具调用预算
...
在 ToolNode 执行前仲裁完整工具调用批次。它把重复调用检测与单工具 occurrence 预算保持为两种独立语义，只准入确定性的可执行前缀；某个工具耗尽时，其他工具仍可继续执行。

Warning 是中性的 advisory，会等已有 ToolMessage 配对完成后再注入。相同 checkpoint receipt 的重放不会重复消费预算。只有相同调用集合达到 hard stop 时才执行一次无工具 finalization；单工具预算耗尽只产生非终止 observation，不会阻止文件写入和交付等其他已准入工具。

**配置**：平台设置 `/admin/settings/system` 中的 System Runtime Policy；每个已准入 Run 使用冻结值。
