...
12. `ToolCallControl` — 仲裁重复调用与 Run 内部工具调用 hard limit
...
在 ToolNode 执行前仲裁完整工具调用批次。它把重复调用检测与单一内部工具调用 hard limit 保持为两种独立语义，并只准入确定性的可执行前缀。所有内部工具统一计数；同一 Run 的 Lead 与所有 Sub-Agent 共用一个计数，不同 Run 相互独立。

相同 checkpoint receipt 的重放不会重复消费上限。达到共享 hard limit 后，该 Run 不再准入新的内部工具调用。

**配置**：平台设置 `/admin/settings/system` 中 System Runtime Policy 的单一内部工具调用 hard limit；每个已准入 Run 使用冻结值。
