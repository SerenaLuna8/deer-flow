...
12. `SubagentLimitMiddleware` — 限制并行子 Agent 调用（如果启用子 Agent）
13. `LoopDetectionMiddleware` — 打破重复工具调用循环
14. `TokenBudgetMiddleware` — 每次运行的 token 预算限制（如果启用）
15. 自定义中间件（如有）
16. `SafetyFinishReasonMiddleware` — provider 安全终止后抑制工具执行（如果启用）
17. `ClarificationMiddleware` — 拦截澄清请求（始终最后）
