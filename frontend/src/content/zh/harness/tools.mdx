...
<Callout type="info" emoji="🔧">
  工具是 Lead Agent 可以采取的行动。ActWeave 提供内置工具、社区集成、MCP
...
```
GroundRoute 是一个元搜索层：一个 API 接入六个搜索引擎（Serper、Brave、Exa、Tavily、Firecrawl、Perplexity）。它会在合格引擎之间路由，并在引擎故障时自动切换。
...
使用 Browserless `/screenshot` 渲染网页并截图，然后通过 ActWeave 的 artifact
