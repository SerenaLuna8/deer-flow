...
在调用模型之前，每个活跃的中间件都有机会修改状态。例如，`DynamicContextMiddleware` 读取 Run 固定的 Memory 来源，并加入一条隐藏的低权限 Human message；`SummarizationMiddleware` 在 token 预算超出时可能压缩当前 Thread 的旧消息。
...
`ThreadState` 更新为新消息、产出物和 durable Thread Context。如果配置了检查点，状态将被持久化。长期 Memory 不是 Thread state 队列：符合条件的 Run 成功后，结算流程会为独立的 Worker/Scheduler 链路准入 durable Source 和 `memory_extract` Job。
