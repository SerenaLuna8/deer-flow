...
disclosure, which collapses only that reasoning body. Thinking, process output,
tool calls, subagent cards, and finalized file cards remain in canonical
model-call order during execution and after completion. They are never moved
into a turn-level **Execution details · N steps** disclosure. Replay or reload
reconstructs the same order, and `present_files` transition prose is not
repeated.
