...
description: The ActWeave App workspace is a browser-based interface for having multi-turn conversations with the agent, tracking task progress, viewing artifacts, and managing files.
---
...
The ActWeave App workspace is a browser-based interface for having multi-turn conversations with the agent, tracking task progress, viewing artifacts, and managing files.
...
Use the model picker in the input bar to choose which active model to
use for the current request. Platform administrators manage this list in
**Admin → System Settings → Models**; ordinary users see only safe selector
metadata, not provider settings or Credential references.
...
Complex tasks can pass through several thinking, tool-call, and tool-result
cycles. During execution, every reasoning round remains available and the
current round opens automatically. Once a complex task finishes, all reasoning,
tool calls, and subagent steps move into one default-collapsed **Execution
details · N steps** disclosure before the final answer. Every model call keeps
an independent **Thought (X seconds)** disclosure in its original position
among the tool calls; reasoning is never flattened into a generic execution
step or replaced by the last pass. The terminal model call's reasoning appears
exactly once as the last reasoning step in that process, while the answer and
finalized file cards remain outside it. A direct answer with no preceding
execution history keeps its standalone reasoning disclosure. Replay or reload
reconstructs the same order, and `present_files` transition prose is not repeated.

## Understanding token usage

If token usage display is enabled, ActWeave shows one conversation-level total in
...
Retried requests, failed requests, cached input tokens, reasoning tokens, and
internal calls that do not appear as normal chat messages can all affect the
token scope represented by each view.

Project administrators with usage access can also see a project-wide Token usage
line chart on the **project overview**. It groups settled Runs from all project
members into the latest 24 consecutive UTC hourly buckets by final job settlement
time, zero-filling hours without usage; the current hour is a partial bucket. This
is a different scope from the thread ledger shown in a conversation header.
