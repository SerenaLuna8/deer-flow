...
DeerFlow requires PostgreSQL for LangGraph checkpoint data and application data such as runs, feedback, and thread metadata. Put the connection URL in `DATABASE_URL` and reference it from the unified `database` section:
...
`DATABASE_URL` accepts `postgresql://...` and `postgresql+asyncpg://...` URLs. PostgreSQL drivers are default dependencies; no database extra is needed. A standalone `checkpointer` configuration is not accepted.
