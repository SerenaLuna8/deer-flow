...
The default agent is the Lead Agent with no custom configuration. It loads all globally enabled skills, has access to all configured tools, and uses the active default model selected by a platform administrator in **Admin → System Settings → Models**.
...
The slug is derived from the display name and must be unique within its asset scope. Published versions and explicit project bindings are stored in PostgreSQL.
