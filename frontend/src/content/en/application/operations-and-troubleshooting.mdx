...
DeerFlow requires PostgreSQL. If startup reports a connection, authentication, or timeout error, verify that `DATABASE_URL` is available to the Gateway process and reachable from its network namespace:
...
Test the same URL with your PostgreSQL client, confirm DNS/port/TLS settings, and verify that the database user can create and migrate DeerFlow tables. PostgreSQL drivers are installed by default.
...
Back up DeerFlow files and PostgreSQL:
...
- Regular PostgreSQL dumps with `pg_dump`, including a restore test with `pg_restore` or `psql` appropriate to the dump format.
