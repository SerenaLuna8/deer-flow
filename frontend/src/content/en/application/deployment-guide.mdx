...
PostgreSQL is required for every DeerFlow deployment. Its drivers are included in the default dependencies, so no database-specific `UV_EXTRAS` value is needed.
...
Use a dedicated database and standard PostgreSQL operational controls for production. Legacy provider branches may remain internally until their cleanup task, but they are not selectable through the public configuration.
...
nginx routes project REST and streaming requests to Gateway and all page requests to the frontend. Gateway exposes only the final account/project-scoped API surface.
