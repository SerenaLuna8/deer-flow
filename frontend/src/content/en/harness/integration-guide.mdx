...
description: ActWeave Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.
---
...
<Callout type="info" emoji="🔌">
  ActWeave Harness can be embedded into any Python application. This guide
  covers the integration patterns for using ActWeave as a library inside your
  own system.
</Callout>

ActWeave Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.
...
# Load configuration (reads config.yaml or ACT_WEAVE_CONFIG_PATH;
...
ActWeave Gateway is itself a FastAPI application. You can mount it as a sub-application or router:
...
# Mount the ActWeave gateway router
from deerflow.app.gateway.main import app as gateway_app
app.mount("/actweave", gateway_app)
```
...
ActWeave Harness is built on LangGraph. The Lead Agent is a standard LangGraph graph. You can compose it with your own LangGraph nodes and graphs:
...
```python
import os
os.environ["ACT_WEAVE_CONFIG_PATH"] = "/path/to/my-actweave-config.yaml"
...
```python
from deerflow.config import load_config
load_config(config_path="/path/to/my-actweave-config.yaml")
```

## MCP server integration

ActWeave can expose its agent as an MCP server, allowing other MCP-compatible systems to call it as a tool. Refer to the ActWeave repository for MCP server integration examples.
