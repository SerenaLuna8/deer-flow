...
→ Models**. API Keys are encrypted directly into the owning Model configuration;
they are never stored in `config.yaml` or returned by the API.

Each Run freezes the exact model payload and Secret Generation admitted for that
Run, so later administrative changes do not silently alter admitted work.
