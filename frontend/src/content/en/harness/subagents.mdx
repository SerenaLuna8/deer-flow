...
Fluva ships with two built-in subagents:
...
In addition to the built-in subagents, Fluva supports delegating to external agents through the **Agent Connect Protocol (ACP)**. ACP allows Fluva to invoke agents running as separate processes (including third-party CLI tools wrapped with an ACP adapter).
...
<Callout type="tip">
  ACP agents run as child processes managed by Fluva. They communicate over
...
Custom agents created through the Fluva App UI can also be invoked as subagents using the `task` tool. When you specify `agent="my-custom-agent"`, the runtime loads that agent's configuration (skills, tool groups, model) and runs it as a subagent for the delegated task.
