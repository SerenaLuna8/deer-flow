...
ActWeave ships with two built-in subagents:
...
- `max_concurrent_subagents`: maximum parallel task calls in one turn (default: 3; enforced range: 1–4)

If the agent tries to call more subagents than the limit allows, the middleware trims the excess
calls. Each Run also has an independent total delegation cap (default: 6; configurable range:
1–50), preventing unbounded serial delegation across multiple turns.

The subtask card shows the effective model used for the delegation. When global Token usage
display is enabled, it also shows the latest cumulative Token total. Live events and restored
structured task results use the same metadata fields.

## ACP agents (external agents)

In addition to the built-in subagents, ActWeave supports delegating to external agents through the **Agent Connect Protocol (ACP)**. ACP allows ActWeave to invoke agents running as separate processes (including third-party CLI tools wrapped with an ACP adapter).
...
<Callout type="tip">
  ACP agents run as child processes managed by ActWeave. They communicate over
...
Custom agents created through the ActWeave App UI can also be invoked as subagents using the `task` tool. When you specify `agent="my-custom-agent"`, the runtime loads that agent's configuration (skills, tool groups, model) and runs it as a subagent for the delegated task.
