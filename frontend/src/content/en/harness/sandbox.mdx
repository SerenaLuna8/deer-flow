...
<Callout type="info" emoji="📦">
  ...
  command-based work. It is what makes Fluva capable of real action, not just
  conversation.
</Callout>
...
Fluva supports four sandbox modes. Choose the one that fits your deployment:
...
<Callout type="warning">
  With the Docker runtime, Worker and the Docker daemon must resolve Run Skill
  mount sources through the same absolute filesystem view. A distinct view
  disables those read-only mounts and has no configuration override.
</Callout>
...
Commands run in an [E2B](https://e2b.dev) cloud micro-VM. Each thread is bound to its own sandbox via metadata, so the same Fluva thread keeps reaching the same e2b sandbox even across gateway restarts and across processes. The cloud VM is reaped automatically by e2b once the configured idle timeout elapses.
...
<Callout type="info">
  Because e2b is a remote sandbox, Fluva's `/mnt/user-data` virtual prefix is
...
Run the optional service independently from `sandbox/provisioner/`. It manages the Pod and Service lifecycle for each Sandbox ID and is not an application deployment target.
...
| `.fluva-flow/threads/{thread_id}/user-data/` | `/mnt/user-data/`                            | Read-write |
...
`LocalSandboxProvider` resolves `host_path` from the Worker host process. If the configured path is not visible to that process, Fluva
