...
<Callout type="info" emoji="📦">
  ...
  command-based work. It is what makes ActWeave capable of real action, not just
  conversation.
</Callout>
...
ActWeave supports four sandbox modes. Choose the one that fits your deployment:
...
<Callout type="info">
  Because e2b is a remote sandbox, ActWeave's `/mnt/user-data` virtual prefix is
...
If the configured `host_path` is not visible to the gateway process, ActWeave
