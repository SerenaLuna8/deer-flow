...
After deployment, a system administrator creates a configuration in
`/admin/settings/models`, stores bounded provider settings, and writes its
encrypted API Key there. The browser never accepts a Python class path and never
receives a stored provider key.
