...
12. `ToolCallControl` — arbitrates repeated calls and Run/Task internal tool-call hard limits
...
Arbitrates the complete tool-call batch before ToolNode execution. It keeps repeated-call detection separate from internal tool-call hard limits and admits only the deterministic executable prefix.

System Runtime Policy v6 uses separate count bindings for the Lead Agent and Sub-Agent Tasks. The Lead count persists throughout one Run, and the `task` delegation call itself counts against the Lead. Every Sub-Agent Task starts with a count bound to its own execution identity, so parallel Tasks never consume one another's allowance and there is no additional Run-wide aggregate. Replaying the same checkpoint receipt does not consume allowance again in its exact binding. Reaching a limit blocks later internal tool calls only in that binding.

**Configuration**: `internal_tool_call_limits.lead_per_run` and `internal_tool_call_limits.subagent_per_task` in System Runtime Policy at `/admin/settings/system`; their defaults are `200` and `50`, respectively, and they are frozen for newly admitted Runs. Already admitted Runs and retained v2–v5 policies keep their frozen legacy limit shared by the Lead and all Sub-Agents; they are not converted to v6 counting.
...
summarization:
  ...
  # Trigger — summarization runs when the estimated context token count
  # reaches this threshold (null = no automatic trigger)
  trigger_tokens: 320000

  # How much recent history to keep after summarization (token count only)
  keep:
    type: tokens
    value: 64000 # keep up to 64000 tokens of recent history
...
**Keep policy**: `tokens` is the only supported type. After summarization the middleware preserves up to `value` tokens of the most recent history (default `64000`). The retired message-count and context-fraction measurements are no longer accepted.
