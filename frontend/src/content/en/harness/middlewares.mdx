...
12. `SubagentLimitMiddleware` — limits parallel subagent calls (if subagents enabled)
13. `LoopDetectionMiddleware` — breaks repetitive tool call loops
14. `TokenBudgetMiddleware` — per-run token budget enforcement (if enabled)
15. Custom middlewares (if any)
16. `SafetyFinishReasonMiddleware` — suppresses tool execution after safety-terminated responses (if enabled)
17. `ClarificationMiddleware` — intercepts clarification requests (always last)
