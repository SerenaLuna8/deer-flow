...
12. `ToolCallControl` — arbitrates repeated calls and per-tool call budgets
...
Arbitrates the complete tool-call batch before ToolNode execution. It keeps repeated-call detection separate from per-tool occurrence budgets, admits only the deterministic executable prefix, and leaves unrelated tools available when one tool is exhausted.

Warnings are neutral advisories queued until existing tool results are paired. Replaying the same checkpoint receipt does not consume the budget again. Only an identical-call hard stop triggers one tool-free finalization; exhausting one tool records a non-terminal observation and does not block other admitted tools such as file writing and delivery.

**Configuration**: System Runtime Policy in `/admin/settings/system`, frozen for each admitted Run.
