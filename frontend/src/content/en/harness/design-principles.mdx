...
provider settings, default selection, and model-owned encrypted API Keys live in
PostgreSQL System Settings. Operators can change behavior without touching code,
while each admitted Run freezes the exact model payload and Secret Generation.
...
Model provider keys are encrypted inside their Model configurations and are
decrypted only at the execution boundary; they are never `config.yaml`
