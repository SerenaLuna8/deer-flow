...
description: Understanding the design principles behind ActWeave Harness helps you use it effectively, extend it confidently, and reason about how your agents will behave in production.
---
...
<Callout type="info" emoji="🏗️">
  ActWeave is built around one central idea: agent behavior should be composed
  from small, observable, replaceable pieces — not hardcoded into a fixed
  workflow graph.
</Callout>

Understanding the design principles behind ActWeave Harness helps you use it effectively, extend it confidently, and reason about how your agents will behave in production.
...
ActWeave is a harness because it bundles:
...
ActWeave is designed for tasks that require more than a single prompt-response exchange. A useful long-horizon agent must:
...
Every architectural decision in ActWeave is evaluated against this use case. Short, stateless exchanges are easy. Long, multi-step workflows under real-world pressure are the target.

## Middleware chain over inheritance

ActWeave does not ask you to subclass an agent or override methods to change its behavior. Instead, it uses a **middleware chain** that wraps every LLM turn.
...
Skills also make the system extensible. Adding a new capability to ActWeave means writing a new skill pack, not modifying the agent core.

## Sandbox is the execution environment

ActWeave gives agents a **sandbox**: an isolated workspace where they can read files, write outputs, run commands, and produce artifacts.
...
Isolation is important because execution should be reproducible and controllable. The sandbox is the reason ActWeave can support genuine action rather than only conversation.
...
ActWeave addresses this through **context engineering** — deliberate control of what the agent sees, remembers, and ignores at each step:
...
- **Memory injection**: one bounded, Run-frozen cross-session document is injected as hidden low-authority Human data, never as a System instruction.

This is one of the most important ideas in ActWeave. Good agent behavior is not only about a stronger model. It is also about giving the model the right working set at the right time.

## Explicit configuration authorities drive behavior

ActWeave separates configuration by authority. Infrastructure and process
policy live in `config.yaml`; versioned model definitions, capabilities,
provider settings, default selection, and exact Credential references live in
PostgreSQL System Settings. Operators can change behavior without touching
code, while each admitted Run freezes the exact model version it will use.

This design principle has three implications:

1. **Reproducibility**: process configuration plus immutable Run snapshots describe the effective behavior at a point in time.
2. **Deployability**: the same code can use different infrastructure and governed model catalogs in different environments.
3. **Auditability**: model changes are versioned database operations rather than untracked YAML edits.

Model provider keys are encrypted System Credentials. They are selected by
exact version in `/admin/settings/models` and decrypted only at the execution
boundary; they are not model settings or `config.yaml` environment references.
...
| Explicit authorities        | Process YAML and versioned database settings have separate roles |
