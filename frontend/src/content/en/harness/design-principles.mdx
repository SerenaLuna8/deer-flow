...
description: Understanding the design principles behind Fluva Harness helps you use it effectively, extend it confidently, and reason about how your agents will behave in production.
---
...
<Callout type="info" emoji="🏗️">
  Fluva is built around one central idea: agent behavior should be composed
  from small, observable, replaceable pieces — not hardcoded into a fixed
  workflow graph.
</Callout>

Understanding the design principles behind Fluva Harness helps you use it effectively, extend it confidently, and reason about how your agents will behave in production.
...
Fluva is a harness because it bundles:
...
Fluva is designed for tasks that require more than a single prompt-response exchange. A useful long-horizon agent must:
...
Every architectural decision in Fluva is evaluated against this use case. Short, stateless exchanges are easy. Long, multi-step workflows under real-world pressure are the target.

## Middleware chain over inheritance

Fluva does not ask you to subclass an agent or override methods to change its behavior. Instead, it uses a **middleware chain** that wraps every LLM turn.
...
Skills also make the system extensible. Adding a new capability to Fluva means writing a new skill pack, not modifying the agent core.

## Sandbox is the execution environment

Fluva gives agents a **sandbox**: an isolated workspace where they can read files, write outputs, run commands, and produce artifacts.
...
Isolation is important because execution should be reproducible and controllable. The sandbox is the reason Fluva can support genuine action rather than only conversation.
...
Fluva addresses this through **context engineering** — deliberate control of what the agent sees, remembers, and ignores at each step:
...
This is one of the most important ideas in Fluva. Good agent behavior is not only about a stronger model. It is also about giving the model the right working set at the right time.

## Explicit configuration authorities drive behavior

Fluva separates configuration by authority. Infrastructure and process
