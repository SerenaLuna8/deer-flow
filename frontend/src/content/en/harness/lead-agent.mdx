...
write-only API Keys, activation, and the default selection under **Admin →
System Settings → Models**. A Run freezes the exact selected model payload and
Secret Generation at admission.
...
thinking options on the model settings page. API Keys remain encrypted model-owned
secret material and are not returned by the API.
