...
Before the model is called, each active middleware has a chance to modify the state. For example, `DynamicContextMiddleware` reads the Run-frozen Memory document snapshot and adds a hidden low-authority Human message, while `SummarizationMiddleware` may condense old Thread messages if the token budget is exceeded.
...
`ThreadState` is updated with new messages, artifacts, and durable Thread context. If a checkpointer is configured, the state is persisted. Long-term Memory is not a Thread-state queue: SNIP archives validated compaction output as pending history, and a separate Worker handles durable `memory_dream` Jobs.
...
A System Model's `max_tokens` limits generation for one provider call; it is
not the cumulative limit for the whole Run. When the Lead Agent in a Private
Run reaches that limit without producing tool intent or structured output,
ActWeave performs at most one recovery call in the same Run. It keeps the same
frozen model configuration, disables thinking and tools, and asks the model to
form a concise, complete final answer from the original task and any visible
partial answer. It does not claim to continue hidden reasoning.

If the truncated response contains tool intent or structured output, or if the
single recovery call is still truncated or has no visible answer, automatic
recovery stops and the UI reports **Model output limit reached**. A failed turn
with no tool activity also offers **Retry without deep thinking**. ActWeave
does not automatically replay tool activity that may have side effects.
