...
Before the model is called, each active middleware has a chance to modify the state. For example, `DynamicContextMiddleware` reads the Run-frozen Memory source and adds a hidden low-authority Human message, while `SummarizationMiddleware` may condense old Thread messages if the token budget is exceeded.
...
`ThreadState` is updated with new messages, artifacts, and durable Thread context. If a checkpointer is configured, the state is persisted. Long-term Memory is not a Thread-state queue: after an eligible Run succeeds, settlement admits a durable Source and `memory_extract` Job for the separate Worker/Scheduler pipeline.
