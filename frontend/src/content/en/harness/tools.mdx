...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. ActWeave provides built-in
...
ActWeave organizes tools into four categories:
...
```
GroundRoute is a meta search layer: one API in front of six engines (Serper, Brave, Exa, Tavily, Firecrawl, Perplexity). It routes across eligible engines and fails over if one is down.
...
through ActWeave's artifact system. This is useful for visual evidence, UI checks,
