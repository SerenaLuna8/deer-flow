...
export const zhCN: Translations = {
  ...
  knowledge: {
    ...
    page: {
      ...
      title: "知识库",
      description: "管理项目知识库与文档，并测试检索效果。",
    },
    tabs: {
      bases: "知识库",
      search: "检索测试",
    ...
    bases: {
      ...
      deleteDescription: (name) =>
        `将删除「${name}」及其全部文档、分段与对象存储文件。该操作不可撤销。`,
      deleteConfirm: "确认删除",
      deleteError: (message) => `删除失败：${message}`,
      documentCount: (count) => `${count} 篇文档`,
      ...
      rebuildOutcome: (accepted, skipped) =>
        skipped > 0
          ? `已受理 ${accepted} 个文档重嵌入；跳过 ${skipped} 个从未发布的文档（可通过重试从原文件解析）。`
          : `已受理 ${accepted} 个文档重嵌入。`,
      rebuildConfirmTitle: "重新嵌入知识库文档",
    ...
    wizard: {
      ...
      uploadProcessingTitle: "处理文档",
      uploadAction: "上传并处理",
      existingConfigurationHint: "沿用知识库的模型与检索配置，修改请前往设置。",
      configuredModelUnavailable: "已配置，模型名称暂不可用",
      retryFailedUploads: "仅重试失败文件",
      refreshStalePreview: "刷新过期预览",
      heroTitle: "创建你的第一个知识库",
      uploadCreateTitle: "上传文档创建知识库",
      uploadCreateHint: "上传文档，自动完成分段与索引，最快的上手方式。",
      orSeparator: "或",
      emptyCreateTitle: "创建一个空知识库",
      emptyCreateHint: "先建库，之后再上传文档。",
      steps: {
        source: "选择数据源",
        configure: "分段与模型",
        finish: "处理并完成",
      ...
      sourceSectionTitle: "上传文本文件",
      dropzoneTitle: "拖拽文件至此，或点击选择文件",
      removeFile: (name) => `移除 ${name}`,
      filesSelected: (count) => `已选择 ${count} 个文件`,
      availableFormats: (extensions) => `当前可用格式：${extensions}`,
      capabilitiesLoading: "正在加载可用文件格式…",
      capabilitiesFailed: "无法加载文件格式。",
      retryFormats: "重试格式列表",
      fileUnavailable: (name) => `${name} 当前无法处理`,
      unsupportedFormatReason: "暂不支持该文件格式。",
      dependencyUnavailableReason: "该格式所需的解析依赖不可用。",
      sandboxUnavailableReason: "本地解析运行环境不可用。",
      tokenizerUnavailableReason: "知识库 Tokenizer 不可用。",
      unknownUnavailableReason: (code) => `该格式当前不可用（${code}）。`,
      next: "下一步",
      previous: "上一步",
      ...
      summaryTitle: "创建设置",
      goToDocuments: "前往文档",
      uploadFailedNote: "以下文件上传失败，可直接重试：",
      previewTitle: "分段预览",
      previewHint: (fileName) => `预览文件：${fileName}`,
      previewPickFile: "选择预览文件",
      ...
      previewChildCount: (count) => `${count} 个子块`,
      previewChildLabel: (index) => `C-${String(index).padStart(2, "0")}`,
      headerSettingsTitle: "表头设置",
      headerModeLabel: (source) => `${source} 的表头模式`,
      headerRowLabel: (source) => `${source} 的表头行`,
      headerCsvSource: "CSV",
      headerAuto: "自动识别",
      headerNone: "无表头",
      headerExplicit: "指定行",
      headerCandidateRow: (row) => `候选第 ${row} 行`,
      headerSelectedRow: (row) => `已选第 ${row} 行`,
      headerNotSelected: "未选择表头",
      actualProfile: (size, unit) => `实际处理参数：${size} ${unit}`,
      knowledgeTokenShort: "知识库 Token",
      characterUnit: "字符",
    },
    detail: {
      navLabel: "知识库导航",
    ...
    metadata: {
      title: "元数据字段",
      description: "定义可赋给文档的自定义字段，检索时可按字段过滤。",
      empty: "还没有元数据字段。",
      addButton: "添加字段",
      addTitle: "添加元数据字段",
      nameLabel: "字段名称",
      namePlaceholder: "例如：部门、发布年份",
      typeLabel: "字段类型",
      typeString: "文本",
      typeNumber: "数字",
      typeTime: "时间",
      ...
      rename: "重命名",
      renameTitle: (name) => `重命名字段「${name}」`,
      delete: "删除",
      deleteTitle: "删除元数据字段",
      deleteDescription: (name) =>
        `将删除字段「${name}」，并清除所有文档上的该字段值。该操作不可撤销。`,
    },
    documents: {
      title: (baseName) => `${baseName} · 文档`,
      empty: "该知识库还没有文档，点击上传开始。",
      filteredEmpty: "没有符合筛选条件的文档。",
      notFound: "该文档不存在或不可访问。",
      backToList: "返回文档列表",
      searchPlaceholder: "搜索文档名或原文件名",
      searchAria: "搜索文档",
      ...
      uploadTitle: "上传文档",
      uploadDescription:
        "支持 PDF、DOCX、TXT、Markdown、CSV、XLSX、HTML、PPTX、EPUB，单文件不超过 50 MB。",
      fileLabel: "文件",
      displayNameLabel: "显示名称（可选）",
      displayNamePlaceholder: "默认使用文件名",
      ...
      chunkingModeGeneral: "通用",
      chunkingModeGeneralHint: "每个分段独立嵌入并直接参与召回。",
      chunkingModeParentChild: "父子分段",
      chunkingModeParentChildHint:
        "父块提供完整上下文，子块用于向量召回；命中子块后返回父块内容。",
      childChunkSizeLabel: "子块最大长度（字符）",
      childChunkSeparatorLabel: "子块分隔符",
      childChunkSeparatorHint: "父块内按此分隔符切出子块（默认 \\n）。",
      ...
      uploadResultSuccess: (name) => `${name}：上传成功`,
      ...
      selectRowAria: (name) => `选择 ${name}`,
      selectedCount: (count) => `已选 ${count} 个文档`,
      batchEnable: "启用",
      batchDisable: "停用",
      batchDelete: "删除",
      clearSelection: "取消选择",
      batchDeleteTitle: "删除所选文档",
      batchDeleteDescription: (count) =>
        `将删除所选 ${count} 个文档及其全部分段与原文件。该操作不可撤销。`,
      metadataAction: "元数据",
      metadataTitle: (name) => `编辑元数据 · ${name}`,
      metadataEmpty: "该知识库还没有元数据字段，请先在「元数据」中定义。",
      metadataClearHint: "留空表示清除该字段的值。",
      processingSummary: {
        processing: (count) => `${count} 个处理中`,
        retryWait: (count) => `${count} 个等待重试`,
        failed: (count) => `${count} 个失败`,
        ready: (count) => `${count} 个就绪`,
      },
      progress: {
        kinds: {
          ingest_document: "摄取",
        ...
        attempt: (current, max) => `第 ${current}/${max} 次尝试`,
        retryWaitAt: (time) => `等待自动重试 · ${time}`,
        retryWaitSoon: "等待自动重试",
        failedDuring: (stage) => `失败于「${stage}」`,
      },
      batchMetadata: "编辑元数据",
      batchMetadataTitle: (count) => `批量元数据（${count} 个文档）`,
      batchMetadataDescription:
        "每个字段可选择保持、设置或清空；仅提交明确修改的字段，对所选文档一次性全部生效或全部回滚。",
      batchMetadataMixedValues: (count) => `${count} 个不同值`,
      batchMetadataModeKeep: "保持",
      ...
      warningMessages: {
        HEADER_INFERRED: "已自动识别表头",
        IMAGE_CORRUPT: "图片无法安全解码",
        IMAGE_LIMIT_EXCEEDED: "图片超过安全上限",
        EXTERNAL_IMAGE_NOT_FETCHED: "按策略未获取外部图片",
        ENCODING_DETECTED: "已使用探测到的文本编码",
        FORMULA_CACHE_MISSING: "公式缺少已保存的计算结果",
        SOURCE_POSITION_UNAVAILABLE: "解析器未提供来源位置",
        TABLE_STRUCTURE_UNAVAILABLE: "解析器未提供表格结构",
        IMAGE_FIRST_FRAME_ONLY: "多帧图片仅保存首帧",
        IMAGE_ANCHOR_UNAVAILABLE: "图片未提供单元格锚点",
      },
      viewPublishedSegments: "查看已发布分段",
    },
    segments: {
      title: (documentName) => `${documentName} · 分段`,
      empty: "该文档还没有分段。",
      position: (position) => `分段 #${position}`,
      pageInfo: (page, pageCount, total) =>
      ...
      stats: (segments, words) =>
        `${segments} 个分段 · ${words.toLocaleString()} 字符`,
      add: "新增分段",
      addTitle: "新增分段",
      addDescription:
      ...
      deleteDescription: (position) =>
        `将删除分段 #${position} 及其向量。该操作不可撤销。`,
      enableAria: (position) => `启用分段 #${position}`,
      disableAria: (position) => `停用分段 #${position}`,
      wordCount: (count) => `${count.toLocaleString()} 字符`,
      manualBadge: "手工添加",
      locatedTitle: (position) => `已定位分段 #${position}`,
      locateFailed: "无法定位该分段，可能已被删除或不可访问。",
      locateStale: "该分段属于较早的文档版本，当前内容可能已更新。",
      dismissLocate: "关闭定位",
      tokenCount: (count) => `${count.toLocaleString()} 知识库 Token`,
      sourceRoles: { source: "来源", context_prefix: "上下文" },
      publishedReadonly: "此前已发布内容（只读）",
      savedImages: (count) =>
        `已保存 ${count} 张图片；图片文字尚未识别，也未进入文本检索。`,
      attachmentInfo: (altText, mediaType, width, height) =>
        `${altText || "图片"} · ${mediaType} · ${width}×${height}`,
      currentDocumentImages: "当前文档图片",
      documentImagesLoading: "正在加载当前文档图片…",
      documentImagesEmpty: "当前文档没有可选择的图片。",
      documentImagesChanged: "文档发布版本已变化，请关闭后重新打开编辑器。",
      insertImage: (index) => `插入图片 ${index}`,
      insertedImageAlt: (index) => `图片 ${index}`,
    },
    sourcePosition: {
      page: (page) => `页 ${page}`,
    ...
    search: {
      ...
      filtersLabel: "元数据过滤",
      filtersHint: "多个条件按「且」组合；仅命中满足全部条件的文档。",
      addFilter: "添加条件",
      removeFilterAria: (index) => `移除条件 ${index}`,
      filterFieldAria: (index) => `条件 ${index} 字段`,
      filterOperatorAria: (index) => `条件 ${index} 操作符`,
      filterValueAria: (index) => `条件 ${index} 值`,
      filterValuePlaceholder: "过滤值",
      filterNoFields: "该知识库还没有元数据字段，无法添加过滤条件。",
      operators: {
        eq: "等于",
        contains: "包含",
        gte: "≥",
      ...
      emptyReasons: {
        not_ready: "目标库中还没有可检索的就绪文档。",
        no_candidates: "本次查询没有召回任何候选。",
        filtered_out: "所有候选都被分数阈值或元数据过滤条件排除。",
        stale_candidates: "检索期间候选内容发生变化，请重新检索。",
      },
      diagnostics: {
        title: "诊断信息",
      ...
      detail: {
        title: "分段详情",
        conflict: "文档在检索后已更新，请重新检索以查看当前内容。",
        staleContent: "该存储行相对当前文档代次已过期。",
        disabledBadge: "已停用",
        matchedChildrenTitle: "本次命中的子块",
        childrenTitle: (total) => `子块（共 ${total} 个）`,
        matchedBadge: "本次命中",
        routes: {
          semantic: "语义",
          lexical: "词法",
    ...
    errors: {
      ...
      invalidResponse: "服务器返回了无法解析的数据。",
      incompleteList: "列表未能完整加载，请刷新重试。",
    },
  },
