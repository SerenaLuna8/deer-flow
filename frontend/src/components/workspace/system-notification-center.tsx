...
const STATUS_LABEL: Record<SystemNotification["status"], string> = {
  pending: "待处理",
  redeemed: "已加入",
  revoked: "已撤销",
  expired: "已过期",
...
const ROLE_LABEL: Record<SystemNotification["role"], string> = {
  editor: "编辑者",
  runner: "执行者",
  viewer: "查看者",
};
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="font-medium">项目邀请</p>
              <p className="mt-1 text-sm">
                {notification.actor.email} 邀请你加入
          ...
          <div className="text-muted-foreground mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
            <span>角色：{ROLE_LABEL[notification.role]}</span>
...
export function SystemNotificationCenter({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    toast.success("已加入项目");
  }, [accept.isSuccess, accept.variables?.notificationId]);
      ...
      <SheetContent className="w-full overflow-hidden p-0 sm:max-w-md">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          {notifications.isLoading ? (
            <p className="text-muted-foreground text-sm">正在加载通知…</p>
          ) : notifications.error ? (
            <div role="alert" className="text-destructive text-sm">
              <p>{notificationErrorMessage(notifications.error)}</p>
              <Button
                type="button"
                variant="outline"
                size="sm"
          ...
          {notifications.hasNextPage ? (
            ...
            >
              {notifications.isFetchingNextPage ? "正在加载…" : "加载更多"}
            </Button>
          ) : null}
          {markAllRead.error ? (
          ...
          {accept.error ? (
            <p role="alert" className="text-destructive mt-3 text-sm">
              {notificationErrorMessage(accept.error)}
            </p>
          ) : null}
        </div>
      </SheetContent>
