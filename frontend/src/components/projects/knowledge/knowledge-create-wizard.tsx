...
export function KnowledgeCreateWizard({
  ...
  // Live preview of the first file: debounce parameter edits, refresh on
  // every change while step 2 is visible.
  ...
  useEffect(() => {
    if (
      ...
      // A stale preview (or error) of the previous file/parameters must not
      // keep rendering next to inputs it no longer describes.
      previewReset();
      return;
    }
    const handle = setTimeout(() => {
      previewMutate({
        ...
        // Child fields are hidden in general mode and may hold stale text.
  ...
  const modelDisplayName =
    options.data?.find((option) => option.id === modelConfigurationId)
      ?.display_name ?? "";
    ...
    <section aria-label={wizard.uploadCreateTitle} className="space-y-6">
      ...
      {step === 2 ? (
        ...
        >
          ...
          <div className="space-y-3">
            <h2 className="text-sm font-semibold">{labels.bases.modelLabel}</h2>
            {options.isLoading ? (
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <Button
              type="submit"
              disabled={createBase.isPending || !configureValid}
            >
        ...
        >
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <h2 className="text-sm font-semibold">{wizard.previewTitle}</h2>
            {preview.data ? (
              <span className="text-muted-foreground text-xs tabular-nums">
                {wizard.previewTotal(preview.data.total)}
              </span>
            ) : null}
          </div>
          {previewFile ? (
          ...
          {preview.data ? (
            <ul className="grid max-h-[32rem] gap-3 overflow-y-auto pr-1">
              {preview.data.items.map((chunk) => (
                <li
                  key={chunk.position}
          ...
          <div className="space-y-3">
            ...
            <dl className="divide-border/60 divide-y overflow-hidden rounded-xl border text-sm">
              ...
              <div className="flex items-center justify-between gap-3 px-4 py-2.5">
                ...
                <dd className="tabular-nums">{parsedChunkSize}</dd>
              </div>
              <div className="flex items-center justify-between gap-3 px-4 py-2.5">
                <dt className="text-muted-foreground">
                  {labels.documents.chunkOverlapLabel}
                </dt>
                <dd className="tabular-nums">{parsedChunkOverlap}</dd>
              </div>
              <div className="flex items-center justify-between gap-3 px-4 py-2.5">
                <dt className="text-muted-foreground">
                  {labels.documents.chunkSeparatorLabel}
                </dt>
                <dd className="font-mono text-xs">{chunkSeparator}</dd>
              </div>
              {chunkingMode === "parent_child" ? (
                <>
                  <div className="flex items-center justify-between gap-3 px-4 py-2.5">
                    ...
                    <dd className="tabular-nums">{parsedChildChunkSize}</dd>
                  </div>
                  <div className="flex items-center justify-between gap-3 px-4 py-2.5">
                    <dt className="text-muted-foreground">
                      {labels.documents.childChunkSeparatorLabel}
                    </dt>
                    <dd className="font-mono text-xs">{childChunkSeparator}</dd>
                  </div>
                </>
              ) : null}
              <div className="flex items-center justify-between gap-3 px-4 py-2.5">
