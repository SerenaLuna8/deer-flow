...
- 可选的项目 RAG 知识库（Knowledge）：独立 `actweave-knowledge` 软件包提供文档上传、
  ...
  子块承载向量，命中按父块内最高子块分回卷去重）。分段模式是库级不变量：
  首个上传的文档确定整库模式（`knowledge_bases.chunking_mode`），之后的上传、
  单文档“分段设置”重新解析与失败重试都必须沿用（向导与分段设置页把模式卡片锁定并说明原因，
  不一致的请求被拒绝），只有设置页“分段模式”区块的“切换分段模式”整库重新解析
  （`POST .../bases/{id}/reparse`：一组参数替换所有文档各自的分段参数，任一文档处理中即整体拒绝，
  切换与全部文档的重新解析任务在同一事务受理）能改变它；清空文档后的知识库由下一次上传重新决定；
  文档列表对尚未按当前库模式重新解析的文档给出提示。空白知识库仅填写名称和描述，
  ...
  分段器 `splitter-v3`：父段字符上限 16000（Token 参数对英文/代码真正生效），在同一页面和标题组内
  为普通正文保留有界尾部重叠，包括 PDF 页内正文及超大的文本片段；重叠是最大值，可能更小，且不跨页面、
  标题组、表格、代码或图片边界。回退边界补齐中英文句末与子句标点，超预算的标题路径/表头降级为截短的
  上下文前缀并给出 `CONTEXT_PREFIX_TRUNCATED` / `OVERSIZED_PREFIX_SPLIT` 警告而不再让整篇文档失败。
  非 Markdown 普通来源的符号保留字面可见含义；HTML 原生结构/代码、Word 生成的链接/表格/图片和原生
  Markdown/MDX 继续按各自结构处理。Word 自定义样式只采用明确的大纲级别 0..5（渲染为标题 1..6）及最近
  的明确值，不按视觉样式、Title/Subtitle、编号或 7..9 猜测。Knowledge Token 是固定的本地分段单位，
  不是所选 Embedding 模型的输入 Token；预览不验证模型容量。历史文档保持原参数，旧的冻结 Token 配置
  （包括手工父子分段修改）需显式重新解析；重新解析会替换人工分段、编辑和附件绑定，重新向量化则保留
  现有正文与分段，不会获得新的解析或分段修复。当前未发布身份为 `splitter-v3` + `cleaner-v2` + `adapter-v2`，
  不代表已部署或已完成 Linux/线上模型验证。
