...
> 交付明细见《RAG知识库MVP执行计划》M8 小节。
>
> 决策修订（M9 已交付）：模型配置所有权与 seed 迁至宿主模型注册表
> （`model_providers` / `model_provider_models`：供应商级凭据、
> Embedding/Reranker 类型化拆分、rerank 库级可选），Package 让出模型
> 配置所有权，经宿主注入的 `KnowledgeModelPort` 取模型材料。正文中
> "Package 自管模型配置"相关内容（§2 的模型配置接口、§5 数据模型的
> `knowledge_model_configurations`、§8.2 初始模型配置与
> `secret_adapter.py`）仅作历史存档；其余章节按当前实现维护。
> 见执行计划 M9 小节。
...
功能面构造入口为 `create_knowledge_module(settings=..., session_factory=..., secret_port=...)`；Project retention 另用不依赖 Secret 或模型客户端的 `create_knowledge_project_purger(settings=..., session_factory=...)`。两者复用宿主 SQLAlchemy session factory；前者创建完整 MinIO/Embedding/Reranker Runtime，后者只保留删除历史对象与行所需的最小能力。Package 还公开不拥有事务的 `purge_knowledge_query_history(session, project_id, owner_user_id)`：宿主 Phase B 在既有 retention authority 与事务内调用，owner 非空时只清该 owner 的原始查询文本，`None` 仅用于 Project 全量清理；它不级联触碰共享 Knowledge 数据。
...
- Knowledge 启用时，现有 Worker 进程调用 `KnowledgeModule.run_worker(stop_event)` 处理后台任务，与主 Worker 共享启停信号和生命周期，不增加独立 Knowledge 进程；关闭时不启动该 Task worker，但仍组合独立 Project purger。
- `run_tool.py` 把当前 Project id、Run owner 和事务内可重验证的执行 authority 注入
  `knowledge_search` 工具；这些字段都不进入模型可见参数。
- Project 删除流程调用独立 purger 的 `purge_project(project_id)`；它不以 `knowledge.enabled` 判断是否存在历史数据。清理未完成，或 Document 行/未成功 exact-key 清理 Task 存在但删除所需 MinIO 配置缺失时，由宿主 retention Job 失败重试，不得继续最终 Project purge。
...
| TaskWorker | claim、执行和重试摄取、资源删除与 exact-key 对象清理任务 |
| ProjectPurger | 独立于功能启停清理 Project 的 MinIO 对象与 Knowledge 行；缺少必要存储配置时失败关闭 |
| PostgreSQLStore | 八张 Knowledge 表的数据访问 |
| MinioObjectStore | 以单 PUT 上传对象、下载到任务临时 Path、校验 bucket 删除语义并执行 exact-key/Project-prefix 删除 |
| KnowledgeModelClient | SiliconFlow 文本 `/embeddings` 与 `/rerank` 调用，按各自 batch 上限分批 |

## 5. 数据模型

当前 Schema V1 固定八张表，全部位于现有 `public` Schema：

1. `knowledge_model_configurations`
2. `knowledge_bases`
3. `knowledge_documents`
4. `knowledge_metadata_fields`
5. `knowledge_segments`
6. `knowledge_segment_children`
7. `knowledge_queries`
8. `knowledge_tasks`

一个 Knowledge Model Configuration 行同时保存共享 Base URL、Embedding model、dimension、batch、Reranker model、Reranker batch 和一个共享的当前加密 API Key。处理规则和 MinIO object key 直接保存在 Knowledge Document，embedding 保存在 Knowledge Segment 或 parent-child 子块。任务 claim、lease、重试计数及仅 object-only cleanup 使用的精确 `storage_key` 直接保存在 Knowledge Task。独立软件包不对应独立 PostgreSQL Schema，不扩展现有 public-only Schema V1 工具链。
...
3. 锁定 Document；仅当它仍保持初始 version 和 `uploading` 状态时更新为 `queued` 并创建 `ingest_document` Task。并发删除递增 version 后必须获胜，上传方清理刚写入的对象且不得复活 Document。
...
HTTP 与 Agent 检索日志都绑定可信 `owner_user_id`。最近查询按
`project_id + owner_user_id + base_id` 过滤，原始 query 属于发起者私有内容，
不作为 Project 共享历史。检索在 Provider 调用前后通过宿主 authority 重验证；
撤权后的最终结果不向调用者返回。

### 6.6 删除

- 删除 Document：标记 `deleting`，Worker 删除 MinIO 对象，再删除 Document；Segment 和 embedding 由数据库级联删除。
- 上传与删除并发时，删除状态和 version 始终获胜。若删除 Worker 已移除行后上传 put
  才完成，且请求内对象清理又失败，Gateway 写入独立
  `delete_document_object` Task，持久化可信 Project/Document id 与精确 `storage_key`，
  可与仍为 running 的普通 `delete_document` Task 并存。Base 尚存时同时重建
  `deleting` Document tombstone；对象清理最终失败会通过 `delete_error` 展示，用户可用
  普通删除入口创建新 Task 重试。Worker 在删除前验证 key 精确属于 Task 的 Project 和
  Document，绝不靠 prefix 猜测目标。
...
- 删除 Project：Worker 的 retention purge 在最终数据库清理前调用独立 `ProjectPurger.purge_project`；它先恢复该 Project 的过期 Knowledge Task lease，再锁定全部 open Task。仍有 `running` handler 时本轮返回未完成；`queued|retry_wait` 在任何对象或关系行删除前移除。随后按 Document 行执行对象优先清理，再清扫数据库签发的 `projects/{project_id}/knowledge/` prefix，覆盖无行晚到对象和遗留 exact-key Task 指向的对象，最后删除剩余 Knowledge 行。Project purger 不逐条执行 object-only Task handler。近期 `uploading` 行使本轮直接返回未完成，期间不删除对象、关系行或执行 prefix sweep；超过一天 settlement grace 的遗留上传只在本轮转成 `deleting` 并创建 exact-key Task，下一轮才允许真正清理。该判断使用 PostgreSQL 时钟；一天显著长于 MinIO 正常传输/重试窗口，且显著短于 Project 固定 30 天 retention。Knowledge 对象和数据未清完时不继续最终 purge。功能关闭只移除路由、Agent 工具和 Knowledge Task worker，不移除该清理能力；无 MinIO 配置且仍有 Document 或状态不是 `succeeded` 的 object-only Task 时失败关闭。
- Project 进入 `pending_deletion` 后，Knowledge Worker 在 claim 同一事务通过宿主 Project share-lock callback 检查 active 状态；inactive claim 退回 `retry_wait` 60 秒且回退本次 attempt，不启动 handler。用户在 30 天内 restore 后任务自动继续，最终 purge 则删除这些暂停任务并等待此前已运行的 handler 静默。
- MinIO bucket 必须保持 versioning `Off`/未配置，且不能启用 Object Lock。启动 health、每次上传获取单槽 PUT 许可后以及 MinIO-backed Document/Base/Project 删除在外部 I/O 前读取 bucket versioning；`Enabled`、`Suspended` 或缺少 `GetBucketVersioning` 权限都失败关闭，避免创建不可清除的新版本或让 `remove_object` 只写 delete marker 后伪成功。没有任何存储证据的纯元数据 Project 清理是唯一不需要 MinIO 的例外。
- 所有合法上传强制使用单 PUT。`upload_max_bytes` 默认值和硬上限均为 50 MiB；`fput_object` 的 `part_size` 取文件大小与 5 MiB 的较大值并关闭并行 multipart，每个 `MinioObjectStore` 只有一个 PUT 槽，约束 MinIO SDK 的整 part 进程内存。不得提高上限、移除单槽或恢复默认 multipart，否则会放大进程内存，或让崩溃留下的 incomplete multipart upload 无法被普通 `list_objects/remove_object` 发现，Project purge 也无法证明字节已清除。
...
- 删除任务和摄取任务均应幂等。
- 不保存每次 Attempt 的独立历史。
- 请求/任务临时文件读写、MinIO 的 `fput_object`、`fget_object`、`remove_object` 和同步 parser 不直接阻塞事件循环；同步调用通过 cancellation-settling blocking adapter 执行，取消后等待已启动调用结束再释放 claim。
...
`backend/app/knowledge/config.py` 从宿主 `AppConfig.model_extra` 读取可选的 `knowledge` 映射，再使用 Package 导出的 `KnowledgeSettings` 校验；配置块完全缺失也等同于 `enabled=false`。从未启用过的默认关闭配置不包含 MinIO 环境变量；已经写入 Knowledge Document 的部署即使关闭功能，也必须保留原 endpoint、bucket 和凭据，直到相关 Project retention 与 Document 删除完成。若提前移除且仍有 Document 行或未成功的 `delete_document_object` Task，Project purger 返回未完成，宿主进入普通 retention 重试而非伪成功。MinIO bucket 必须关闭 versioning/Object Lock；凭据除对象读写删除外必须允许 `GetBucketVersioning` 和列出 Knowledge Project prefix，供启动校验、无行晚到对象与 Project retention 清扫；状态读取、列举或删除失败均失败关闭。Harness 不 import Knowledge Package，也不把这些启动配置放入 System Runtime Settings。Gateway 与 Worker 使用同一组 MinIO endpoint 和 bucket。MinIO 是 MVP 唯一文件存储实现，不保留本地目录 fallback、存储选择器或多后端字段。模型 endpoint、model 名称、dimension 和 batch size 仍存在数据库模型配置中。
...
- 宿主在 DDL 前使用 `SecretKey`/`SecretEnvelope` 生成 protected material；Package 在八张表创建后、`schema_v1` marker 发布前，以一个事务写入确定性配置。
...
M8 已交付父子分段、自定义分隔符、Segment 治理、模型迁移/重建，以及查询日志与命中统计。仍待后续评估：
