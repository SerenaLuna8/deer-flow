...
`MinioObjectStore` 直接包装官方同步 MinIO client：`upload_from`、`download_to` 和 `delete` 分别调用 `fput_object`、`fget_object` 和 `remove_object`，并统一通过 `asyncio.to_thread` 执行。Gateway/Worker 的临时目录创建、文件写入和清理也使用异步文件操作或 `asyncio.to_thread`。
...
5. 在一个事务中把 Document 更新为 `queued`，并插入 `ingest_document` Task。
6. 上传失败时删除已写对象和 `uploading` Document，并向请求返回错误。
7. 无论成功、失败或取消，Gateway 都删除单次请求临时文件。

默认文件上限为 50 MiB。每次上传创建独立 Document，不做内容去重。
...
Worker 为一次处理创建临时目录，把对象下载到临时 `Path` 后调用 Extractor，并在处理结束后删除临时目录。MinIO I/O 和上述同步 parser 均通过 `asyncio.to_thread` 执行，不直接阻塞 Worker 事件循环。
...
Splitter 优先在段落和换行边界切分；没有合适边界时按字符数切分。输出 position 从 1 连续递增。切分数量超过 `max_segments_per_document`（默认 5000）时本次摄取失败，Document 进入 `failed` 并说明超限。
...
自动执行最多 3 次。前两次失败进入 `retry_wait`；第三次失败进入 `failed`。只有 `ingest_document` 最终失败时才把匹配 version 的 Document 置为 `failed`；删除 Task 失败时资源保持 `deleting`。
