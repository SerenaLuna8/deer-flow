...
本文档是 DeerFlow 项目优先、多用户 SaaS V1 的唯一设计基线。当前实现事实以代码、final PostgreSQL baseline 和自动化发布门禁为准。

M1–M8 已正式完成。M7 已收敛为唯一 final PostgreSQL baseline、project/admin API、project-scoped
frontend、独立 Worker/Scheduler、durable SSE/quota/audit 和 fresh-install 边界；M8 进一步完成完整
...
必须自行创建新空数据库并重新 setup，不能原地 stamp、清空、转换或自动删除旧库。
...
- fresh setup 幂等和旧库 fail-before-DDL；
...
| M7 | 最终 legacy source/API 清理与 fresh-install 收口 | 已完成 |
| M8 | 完整隔离矩阵、安全审查、宿主机验证和发布验收 | 已完成 |

里程碑完成状态以当前代码、数据库 baseline 和自动化门禁证据为准。
...
持久化、业务层授权、强制作用域仓储、数据库复合约束、持久化任务与事件以及可验证的 fresh install。
