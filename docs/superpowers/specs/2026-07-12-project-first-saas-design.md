...
所有里程碑均未开始。文档中的设计决策不表示对应功能已经实现、测试或发布。
...
项目内不提供跨项目下拉切换器。用户通过返回项目工作台切换项目。
...
- 新项目通过显式复制获得内置默认资产，不读取可变的全局在线资产。
...
完成存储层切换后，M1 再建设项目、成员关系、`ProjectContext` 和项目工作台。后续每个业务里程碑继续交付对应数据的项目化回填和验证能力。
...
| 里程碑 | 内容 | 当前状态 |
| --- | --- | --- |
| M1 | 决策冻结、威胁模型、数据清单、SQLite 原样迁移、PostgreSQL 切换、项目基础和项目工作台 | 未开始 |
| M2 | 成员、邀请、角色变更、退出、删除恢复 | 未开始 |
| M3 | Agent、Skill、MCP 版本和凭据审批 | 未开始 |
| M4 | 私有对话、运行、文件、记忆和连接 | 未开始 |
| M5 | 自动化、任务租约、Worker 和持久化 SSE | 未开始 |
| M6 | 配额、审计、平台管理、备份恢复 | 未开始 |
| M7 | 最终迁移、切换、回滚窗口和旧路径清理 | 未开始 |
| M8 | 完整隔离矩阵、安全审查、运维文档和发布验收 | 未开始 |
